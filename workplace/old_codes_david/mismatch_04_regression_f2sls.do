...
/* physical ability */
*  lths univ black hisp
reg hlth_composite I_enterage1-I_enterage8
...
*reg ability_h hispanic black lths univ, vce(robust)
...
* for instruments
* include other skill and ability variables that are taken as exogenous
global hlth_list ${hlth_list} ability_m ability_v skill_m skill_v absmm_m absmm_v
// generate hlth_list_ten
global hlth_list_ten tenure_occ ten_occ2
global hlth_list_ten_iv ten_occ_iv ten_occ2_iv
foreach hv of varlist $hlth_list {
	gen `hv'_ten_occ = `hv'*tenure_occ
	global hlth_list_ten ${hlth_list_ten} `hv'_ten_occ 
	gen `hv'_ten_occ_iv = `hv'*ten_occ_iv
	global hlth_list_ten_iv ${hlth_list_ten_iv} `hv'_ten_occ_iv
}

global hlth_list $hlth_list $hlth_list2
...
* OLD WAY OF DOIGN THINGS
*qui foreach mmv of varlist mm mm_sgn mm_neg mm_pos mm_?? mm_??_neg mm_??_pos absmm_??{
*	reg `mmv' ${hlth_list} , vce(robust)
*	predict `mmv'_iv
*}
...
/////health iv/////
///////////////////
/* create pos neg for h_vi */
gen mm_h_iv_neg = mm_h_iv if mm_h_iv <0
replace mm_h_iv_neg = 0 if mm_h_iv > 0
gen mm_h_iv_pos = mm_h_iv if mm_h_iv >0
...
/////health iv/////
///////////////////
/* re-normalize dimensions for iv - not necessary because doing it above
...
/*------------------------------------------------------------------------------------*/
/* create health instruments for interaction terms */
/*------------------------------------------------------------------------------------*/
* w/o tenure iv
qui foreach mmv of varlist mm_iv mm_pos_iv mm_neg_iv{
	gen `mmv'_ten_occ = `mmv'*tenure_occ
}
* w/ tenure iv
qui foreach mmv of varlist mm_iv mm_neg_iv mm_pos_iv{
	gen `mmv'_ten_occ_iv = `mmv'*ten_occ_iv
}
...
gen ability_mean_iv = (ability_aa + ability_bb + ability_h_iv)/3
gen ability_mean_iv_ten_occ = ability_mean_iv*tenure_occ
gen ability_mean_iv_ten_occ2 = ability_mean_iv*ten_occ2
gen ability_mean_iv_ten_occ_iv = ability_mean_iv*ten_occ_iv
gen ability_mean_iv_ten_occ2_iv = ability_mean_iv*ten_occ2_iv
...
label var mm_iv "Mismatch"
label var mm_iv_ten_occ "Mismatch $\times$ Occ Tenure"
label var mm_neg_iv "Mismatch Negative Components"
label var mm_pos_iv "Mismatch Positive Components"
label var mm_neg_iv_ten_occ "Mismatch Negative $\times$ Occ Tenure"
label var mm_pos_iv_ten_occ "Mismatch Positive $\times$ Occ Tenure"
...
local nlist "aa bb cc h_iv"
local llist "Verbal Math Phys Phys"
local ll = 1
...
gen lmm_iv = mm_iv[_n-1] if switch_occ == 1
replace lmm_iv = lmm_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
...
gen lmm_neg_iv = mm_neg_iv[_n-1] if switch_occ == 1
replace lmm_neg_iv = lmm_neg_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
gen lmm_pos_iv = mm_pos_iv[_n-1] if switch_occ == 1
replace lmm_pos_iv = lmm_pos_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
...
by id: gen cmm_iv = sum(lmm_iv*lten_occ)  if switch_occ==1  & lmm_iv<. & lten_occ<.
by id: gen cmm_neg_iv = sum(lmm_neg_iv*lten_occ)  if switch_occ==1 
by id: gen cmm_pos_iv = sum(lmm_pos_iv*lten_occ)  if switch_occ==1 
replace cmm_iv = cmm_iv/totexp if switch_occ==1 
replace cmm_neg_iv = cmm_neg_iv/totexp if switch_occ==1 
replace cmm_pos_iv = cmm_pos_iv/totexp if switch_occ==1
...
replace cmm_iv = cmm_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
replace cmm_neg_iv = cmm_neg_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
replace cmm_pos_iv = cmm_pos_iv[_n-1] if switch_occ == 0 & id == id[_n-1]
...
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/* estimation with imputed health measure */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/* with alternative definition of health measure */

/* temp */
gen mm_temp = mm
gen mm_ten_occ_temp = mm_ten_occ
gen mm_pos_temp = mm_pos
gen mm_neg_temp = mm_neg
gen mm_pos_ten_occ_temp = mm_pos_ten_occ
gen mm_neg_ten_occ_temp = mm_neg_ten_occ
gen cmm_temp = cmm
gen cmm_pos_temp = cmm_pos
gen cmm_neg_temp = cmm_neg
gen absmm_cc_temp = absmm_cc
gen absmm_cc_ten_occ_temp = absmm_cc_ten_occ
gen absmm_cc_ten_occ_iv_temp = absmm_cc_ten_occ_iv
gen ability_mean_temp = ability_mean
gen ability_mean_ten_occ_temp = ability_mean_ten_occ
gen ability_mean_ten_occ_iv_temp = ability_mean_ten_occ_iv
gen ability_cc_temp = ability_cc
gen ability_cc_ten_occ_temp = ability_cc_ten_occ
gen ability_cc_ten_occ_iv_temp = ability_cc_ten_occ_iv
gen cmm_cc_temp = cmm_cc
 
 /* replace the measures */
replace mm = mm_iv
replace mm_ten_occ = mm_iv_ten_occ
replace mm_pos = mm_pos_iv
replace mm_neg = mm_neg_iv
replace mm_pos_ten_occ = mm_pos_iv_ten_occ
replace mm_neg_ten_occ = mm_neg_iv_ten_occ
replace cmm = cmm_iv
replace cmm_pos = cmm_pos_iv
replace cmm_neg = cmm_neg_iv
replace absmm_cc = absmm_h_iv
replace absmm_cc_ten_occ = absmm_h_iv_ten_occ
replace absmm_cc_ten_occ_iv = absmm_h_iv_ten_occ_iv
replace ability_mean = ability_mean_iv
replace ability_mean_ten_occ = ability_mean_iv_ten_occ
replace ability_mean_ten_occ_iv = ability_mean_iv_ten_occ_iv
replace ability_cc = ability_h_iv
replace ability_cc_ten_occ = ability_h_iv_ten_occ
replace ability_cc_ten_occ_iv = ability_h_iv_ten_occ_iv
replace cmm_cc = cmm_h_iv
...
global xlist  mm_ten_occ
xi: reg lwage $xlist $zlist $xlist_0 mm ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/olsh_mm_means.ster, replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) mm $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/ivh_mm_means.ster, replace

/*------------------------------------------------------------------------------------*/
/* mismatch with positive and negative components */

global xlist  mm_pos_ten_occ mm_neg_ten_occ
xi: reg lwage $xlist $zlist $xlist_0 mm_pos mm_neg ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/olsh_mm_means_pos_neg.ster, replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) mm_pos mm_neg $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/ivh_mm_means_pos_neg.ster, replace

/*------------------------------------------------------------------------------------*/
/* cumulative mismatch */

global xlist  mm cmm mm_ten_occ
xi: reg lwage $xlist $zlist $xlist_0 ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/olsh_cmm_mm_means.ster, replace

global xlist  mm_ten_occ $xlist_0 ability_mean_ten_occ skill_mean_ten_occ
global ivlist mm_ten_occ_iv $ivlist_0 ability_mean_ten_occ_iv skill_mean_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) mm cmm $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/ivh_cmm_mm_means.ster, replace

/*------------------------------------------------------------------------------------*/
/* cumulative mismatch with positive and negative components */

global xlist  mm_neg mm_pos cmm_pos cmm_neg mm_pos_ten_occ mm_neg_ten_occ
xi: reg lwage $xlist $zlist $xlist_0 ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/olsh_cmm_mm_means_pos_neg.ster, replace

global xlist  mm_pos_ten_occ mm_neg_ten_occ $xlist_0 ability_mean_ten_occ skill_mean_ten_occ
global ivlist mm_pos_ten_occ_iv mm_neg_ten_occ_iv $ivlist_0 ability_mean_ten_occ_iv skill_mean_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) mm_neg mm_pos cmm_pos cmm_neg $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(robust)
...
global xlist  absmm_??_ten_occ ability_??_ten_occ skill_??_ten_occ $xlist_0
xi: reg lwage absmm_?? $xlist $zlist ability_?? skill_?? i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/olsh_ind_mm_means.ster, replace

global xlist  absmm_??_ten_occ $xlist_0 ability_??_ten_occ skill_??_ten_occ
global ivlist absmm_aa_ten_occ_iv absmm_bb_ten_occ_iv absmm_cc_ten_occ_iv $ivlist_0 ability_??_ten_occ_iv skill_??_ten_occ_iv 
xi: ivregress 2sls lwage absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d , vce(robust)
...
global xlist  absmm_??_ten_occ ability_??_ten_occ skill_??_ten_occ $xlist_0 
xi: reg lwage cmm_aa cmm_bb cmm_cc absmm_?? $xlist $zlist ability_?? skill_?? i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/olsh_ind_cmm_mm_means.ster, replace
...
xi: reg fswitch_occ mm $prswlist, vce(robust)
estimate save ${result}/lpmh_${diminitls}_mm.ster, replace
...
xi: reg fswitch_occ mm_neg mm_pos $prswlist, vce(robust)
estimate save ${result}/lpmh_${diminitls}_mm_pos_neg.ster, replace

xi: ivregress 2sls fswitch_occ (${prswlist_x} = ${prswlist_iv}) mm_pos mm_neg $prswlist_z, vce(robust)
estimate save ${result}/ivh_lpm_${diminitls}_mm_pos_neg.ster, replace

/*------------------------------------------------------------------------------------*/
/* lpm by components */

xi: reg fswitch_occ absmm_?? $prswlist, vce(robust)
estimate save ${result}/lpmh_${diminitls}_mm_ind.ster, replace
...
estimate use ${result}/olsh_mm_means.ster
estimate store olsh_mm_means
estimate use ${result}/olsh_cmm_mm_means.ster
estimate store olsh_cmm_mm_means
...
/* csv */
esttab ivh_mm_means ivh_cmm_mm_means olsh_mm_means olsh_cmm_mm_means iv_mm_means iv_cmm_mm_means ols_mm_means ols_cmm_mm_means ///
                   using ${result}/table_${diminitls}.csv, b(4) ///
                   r2 nodepvars nogaps not label nonotes drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("IV-H" "IV-H" "OLS-H" "OLS-H" "IV" "IV" "OLS" "OLS") ///
...
estimate use ${result}/olsh_mm_means_pos_neg.ster
estimate store olsh_mm_means_pos_neg
estimate use ${result}/iv_mm_means_pos_neg.ster
estimate store iv_mm_means_pos_neg
estimate use ${result}/ivh_mm_means_pos_neg.ster
estimate store ivh_mm_means_pos_neg

estimate use ${result}/ols_cmm_mm_means_pos_neg.ster
estimate store ols_cmm_mm_means_pos_neg
estimate use ${result}/olsh_cmm_mm_means_pos_neg.ster
estimate store olsh_cmm_mm_means_pos_neg
...
esttab ivh_mm_means_pos_neg ivh_cmm_mm_means_pos_neg olsh_mm_means_pos_neg olsh_cmm_mm_means_pos_neg iv_cmm_mm_means_pos_neg iv_mm_means_pos_neg ols_cmm_mm_means_pos_neg ols_mm_means_pos_neg ///
...
esttab ivh_mm_means_pos_neg ivh_cmm_mm_means_pos_neg olsh_mm_means_pos_neg olsh_cmm_mm_means_pos_neg iv_mm_means_pos_neg iv_cmm_mm_means_pos_neg ols_mm_means_pos_neg ols_cmm_mm_means_pos_neg ///
...
estimate use ${result}/olsh_ind_mm_means.ster
estimate store olsh_ind_mm_means
estimate use ${result}/olsh_ind_cmm_mm_means.ster
estimate store olsh_ind_cmm_mm_means
...
/* csv */
esttab ivh_ind_mm_means ivh_ind_cmm_mm_means olsh_ind_mm_means olsh_ind_cmm_mm_means iv_ind_mm_means iv_ind_cmm_mm_means ols_ind_mm_means ols_ind_cmm_mm_means ///
...
esttab ivh_ind_mm_means ivh_ind_cmm_mm_means olsh_ind_mm_means olsh_ind_cmm_mm_means iv_ind_mm_means iv_ind_cmm_mm_means ols_ind_mm_means ols_ind_cmm_mm_means ///
		   ...
		   order(absmm_aa absmm_bb absmm_cc absmm_aa_t* absmm_bb_t* absmm_cc_t* cmm_aa cmm_bb cmm_cc ability_?? ability_??_* skill_?? skill_??_* ten* exp* oj $zlist) replace
...
esttab ivh_ind_mm_means ivh_ind_cmm_mm_means olsh_ind_mm_means olsh_ind_cmm_mm_means iv_ind_mm_means iv_ind_cmm_mm_means ols_ind_mm_means ols_ind_cmm_mm_means ///
		   ...
		   mtitles("IV-H" "IV-H" "OLS-H" "OLS-H" "IV" "IV" "OLS" "OLS") ///
		   title("Wage Regression with Mismatch by Components (Full Results)") ///
		   order(absmm_aa absmm_bb absmm_cc absmm_aa_t* absmm_bb_t* absmm_cc_t* cmm_aa cmm_bb cmm_cc ability_?? ability_??_* skill_?? skill_??_* ten* exp* oj $zlist) ///
                   star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
...
estimate use ${result}/lpmh_${diminitls}_mm.ster
estimate store lpmh_${diminitls}_mm
...
estimate use ${result}/lpmh_${diminitls}_mm_pos_neg.ster
estimate store lpmh_${diminitls}_mm_pos_neg
estimate use ${result}/ivh_lpm_${diminitls}_mm_pos_neg.ster
estimate store ivh_lpm_${diminitls}_mm_pos_neg
...
estimate use ${result}/lpmh_${diminitls}_mm_ind.ster
estimate store lpmh_${diminitls}_mm_ind
...
/* csv */
esttab ivh_lpm_${diminitls}_mm ivh_lpm_${diminitls}_mm_ind lpmh_${diminitls}_mm lpmh_${diminitls}_mm_ind iv_lpm_${diminitls}_mm iv_lpm_${diminitls}_mm_ind lpm_${diminitls}_mm lpm_${diminitls}_mm_ind ///
                   using ${result}/table_${diminitls}_switch_lpm.csv, b(4) ///
                   nodepvars nogaps not label nonotes ///
		   mtitles("LPM-IVH" "LPM-IVH" "LPM-H" "LPM-H" "LPM-IV" "LPM-IV" "LPM" "LPM") ///
...
esttab ivh_lpm_${diminitls}_mm_pos_neg lpmh_${diminitls}_mm_pos_neg iv_lpm_${diminitls}_mm_pos_neg lpm_${diminitls}_mm_pos_neg ///
                   using ${result}/table_apx_${diminitls}_switch_lpm_sgn.tex, b(4) se(4) ///
                   nodepvars nogaps label longtable substitute(\hline\hline \hline \hline "\noalign{\vskip 1mm} \hline \noalign{\vskip 1mm}" ///
		   "Standard errors in parentheses" "All regressions include occupation and industry dummies." ///
		   "\sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)" "Robust standard errors in parentheses. \sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)." ///
                   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   mtitles("LPM-IVH" "LPM-H" "LPM-IV" "LPM") ///
...
/* get the values back */
replace mm = mm_temp
replace mm_ten_occ = mm_ten_occ_temp
replace mm_pos = mm_pos_temp
replace mm_neg = mm_neg_temp
replace mm_pos_ten_occ = mm_pos_ten_occ_temp
replace mm_neg_ten_occ = mm_neg_ten_occ_temp
replace cmm = cmm_temp
replace cmm_pos = cmm_pos_temp
replace cmm_neg = cmm_neg_temp
replace absmm_cc = absmm_cc_temp
replace absmm_cc_ten_occ = absmm_cc_ten_occ_temp
replace absmm_cc_ten_occ_iv = absmm_cc_ten_occ_iv_temp
replace ability_mean = ability_mean_temp
replace ability_mean_ten_occ = ability_mean_ten_occ_temp
replace ability_mean_ten_occ_iv = ability_mean_ten_occ_iv_temp
replace ability_cc = ability_cc_temp
replace ability_cc_ten_occ = ability_cc_ten_occ_temp
replace ability_cc_ten_occ_iv = ability_cc_ten_occ_iv_temp
replace cmm_cc = cmm_cc_temp
...
twoway (lpolyci fswitch_occ mm_iv, clcolor(blue) clwidth(thick)), ///
...
saving(${result}/fig_pswitch_${diminitls}_ivh_mm, replace)
graph export ${result}/fig_pswitch_${diminitls}_ivh_mm.eps, replace
...
twoway (lpoly fswitch_occ absmm_aa, lcolor(blue) lpattern(solid) lwidth(thick)) || ///
       ...
       (lpoly fswitch_occ absmm_h_iv, lcolor(green) lpattern(dash) lwidth(thick)), ///
ytitle("Probability of Occupational Switch", size(medlarge)) ///
xtitle("Mismatch", size(medlarge)) ///
title("Probability of Occupational Switch", size(large)) ///
legend(lab(1 "Verbal Mismatch") lab(2 "Math Mismatch") lab(3 "Physical  Mismatch") ring(0) pos(12) cols(1)) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ivh_mm_aa_mm_bb_mm_cc, replace)
graph export ${result}/fig_pswitch_${diminitls}_ivh_mm_aa_mm_bb_mm_cc.eps, replace

/* switch rate by mismatch signed*/
twoway (lpolyci fswitch_occ mm_neg_iv, clcolor(blue) lpattern(solid) clwidth(thick)) || (lpolyci fswitch_occ mm_pos_iv, clcolor(red) lpattern(solid) clwidth(thick)), ///
ytitle("Probability of Occupational Switch", size(medlarge)) ///
xtitle("Mismatch", size(medlarge)) ///
title("Probability of Occupational Switch", size(large)) ///
legend(off) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ivh_mm__sgn, replace)
graph export ${result}/fig_pswitch_${diminitls}_ivh_mm_sgn.eps, replace


/* switch rate by mismatch aa and bb cc*/
twoway (lpoly fswitch_occ mm_aa_neg, lcolor(blue) lpattern(solid) lwidth(thick)) || (lpoly fswitch_occ mm_aa_pos, lcolor(blue) lpattern(solid) lwidth(thick))  || ///
       (lpoly fswitch_occ mm_bb_neg, lcolor(red) lpattern(longdash) lwidth(thick)) || (lpoly fswitch_occ mm_bb_pos, lcolor(red) lpattern(longdash) lwidth(thick)) || ///
       (lpoly fswitch_occ mm_h_iv_neg, lcolor(green) lpattern(dash) lwidth(thick)) || (lpoly fswitch_occ mm_h_iv_pos, lcolor(green) lpattern(dash) lwidth(thick)), ///
ytitle("Probability of Occupational Switch", size(medlarge)) ///
xtitle("Mismatch", size(medlarge)) ///
title("Probability of Occupational Switch", size(large)) ///
legend(lab(1 "Verbal Mismatch") lab(3 "Math Mismatch") lab(5 "Physical  Mismatch") lab(2 " ") lab(4 " ")  lab(6 " ") ring(0) pos(12) cols(2)) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ivh_mm_aa_mm_bb_mm_cc_sgn, replace)
graph export ${result}/fig_pswitch_${diminitls}_ivh_mm_aa_mm_bb_mm_cc_sgn.eps, replace
...
legend(off) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26, grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ols_mm, replace)
graph export ${result}/fig_pswitch_${diminitls}_ols_mm.eps, replace
...
legend(lab(1 "Verbal Mismatch") lab(2 "Math Mismatch") lab(3 "Physical  Mismatch") ring(0) pos(12) cols(1)) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ols_mm_aa_mm_bb_mm_cc, replace)
graph export ${result}/fig_pswitch_${diminitls}_ols_mm_aa_mm_bb_mm_cc.eps, replace


/* switch rate by mismatch signed*/
twoway (lpolyci fswitch_occ mm_neg, clcolor(blue) lpattern(solid) clwidth(thick)) || (lpolyci fswitch_occ mm_pos, clcolor(red) lpattern(solid) clwidth(thick)), ///
ytitle("Probability of Occupational Switch", size(medlarge)) ///
xtitle("Mismatch", size(medlarge)) ///
title("Probability of Occupational Switch", size(large)) ///
legend(off) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ivh_mm__sgn, replace)
graph export ${result}/fig_pswitch_${diminitls}_ols_mm_sgn.eps, replace


/* switch rate by mismatch aa and bb cc*/
twoway (lpoly fswitch_occ mm_aa_neg, lcolor(blue) lpattern(solid) lwidth(thick)) || (lpoly fswitch_occ mm_aa_pos, lcolor(blue) lpattern(solid) lwidth(thick))  || ///
       (lpoly fswitch_occ mm_bb_neg, lcolor(red) lpattern(longdash) lwidth(thick)) || (lpoly fswitch_occ mm_bb_pos, lcolor(red) lpattern(longdash) lwidth(thick)) || ///
       (lpoly fswitch_occ mm_cc_neg, lcolor(green) lpattern(dash) lwidth(thick)) || (lpoly fswitch_occ mm_cc_pos, lcolor(green) lpattern(dash) lwidth(thick)), ///
ytitle("Probability of Occupational Switch", size(medlarge)) ///
xtitle("Mismatch", size(medlarge)) ///
title("Probability of Occupational Switch", size(large)) ///
legend(lab(1 "Verbal Mismatch") lab(3 "Math Mismatch") lab(5 "Physical  Mismatch") lab(2 " ") lab(4 " ")  lab(6 " ") ring(0) pos(12) cols(2)) ///
graphregion(color(white)) xlabel(,grid gstyle(dot)) ylabel(0.14(0.04)0.26,grid gstyle(dot)) ///
saving(${result}/fig_pswitch_${diminitls}_ols_mm_aa_mm_bb_mm_cc_sgn, replace)
graph export ${result}/fig_pswitch_${diminitls}_ols_mm_aa_mm_bb_mm_cc_sgn.eps, replace
