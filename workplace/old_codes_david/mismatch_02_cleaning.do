...
use $result/employment_01.dta, clear
                                                                                                                             /* DROP LATER */
*keep if intvw_num < 20
...
save $result/employment_02.dta, replace
...
use $result/weekly_01.dta, clear
...
sort id intvw_num
merge m:1 id intvw_num using $result/employment_02
tab _m
drop if _m == 2
drop _m

/* input hours worked for each job */
...
save $result/weekly_02.dta, replace

/* 4hours.do in Pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/weekly_02.dta, clear
...
save $result/weekly_03.dta, replace

/* 5weeks.do in Pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/weekly_03.dta, clear
...
save $result/weekly_04.dta, replace

/* 6years.do in Pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/employment_02.dta, clear
...
merge m:1 occ1980 using $workplace/occ1980_2_occ1990.dta
...
merge m:1 occ1970 using $workplace/occ1970_2_occ1990.dta
...
replace ind_emp`i' = -5 if ind_emp`i' >= 991 & ind_emp`i' <= 992 & intvw_num == 15 /* public administration */
}
...
replace ind_cps = -5 if ind_cps >= 997 & ind_cps <= 999 & intvw_num == 15 /* public administration */
...
merge m:1 occ2000 using $workplace/occ2000_2_occ1990.dta
...
replace ind_emp`i' = -5 if ind_emp`i' >= 997 & ind_emp`i' <= 999 & intvw_num < 20 & intvw_num != 15 /* public administration */
}
...
replace ind_emp`i' = -5 if   ind_emp`i' == 992                       & intvw_num >= 20 /* public administration */
}
...
save $result/employment_03.dta, replace
...
use $result/demographics_01.dta, clear
*keep if intvw_num < 20                                                                                                      /* DROP LATER */ 
keep if sample_id >= 1 & sample_id <= 4
...
save $result/demographics_02.dta, replace
...
use $result/weekly_04.dta, clear
...
merge m:m id year using $result/demographics_02.dta
ta _m
drop _m
...
gen drop_obs_04 = 0
replace drop_obs_04 =1 if empty == 1
sort id time
...
gen drop_obs_05 = 0
replace drop_obs_05 = 1 if check_mil > 2 & check_mil != .
gen drop_obs_06 = 0
replace drop_obs_06 = 1 if military == 1
...
gen drop_obs_07 = 0
replace drop_obs_07 = 1 if flag_2 == 1	
sort id time
...
/* I drop individuals have been more than one year out of the regular labor force */
...
sort id time
by id: replace skip = 2 if cum_skip[_n-1] == 1 & cum_skip == 2 & nyears >= 10
...
/* delete individuals if they are weekly attached to the labor force within 10 years */
...
gen drop_obs_11 = 0
replace drop_obs_11 = 1 if year == 1977
drop if drop_obs_11 == 1
...
save $result/weekly_05.dta, replace

/* 9mess.do in Pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/weekly_05.dta, clear

sort id intvw_num
merge m:1 id intvw_num using $result/employment_03.dta
ta _m
drop if _m == 2
drop _m
...
quietly replace occ = 347 if occ == 345
...
quietly replace occ = 474 if occ == 484
...
/* this was created by workplace_occ/onetstata/KSA_data_setup.do */
rename occ occ90
merge m:1 occ90 using $base_folder/onetstata/ONET_ASVAB_occ90.dta
rename occ90 occ
drop if _merge == 2
drop _merge
...
save $result/weekly_06.dta, replace

/* 10addio.do in pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/weekly_06.dta, clear

/* convert occ nad ind to 1-digit level */
gen occ_1d = -10
replace occ_1d = 1  if occ >= 003 & occ<=199
replace occ_1d = 2  if occ >= 203 & occ<=391
replace occ_1d = 3  if occ >= 403 & occ<=469
replace occ_1d = 4  if occ >= 473 & occ<=499
replace occ_1d = 5  if occ >= 503 & occ<=699
replace occ_1d = 6  if occ >= 703 & occ<=890
replace occ_1d = 7  if occ >= 903 & occ<=905
replace occ_1d = 8  if occ >  0   & occ_1d<0
...
save $result/weekly_07.dta, replace

/* 11cliow.do */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/weekly_07.dta, clear
...
save $result/yearly_01.dta, replace

/* 12double.do in pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $result/yearly_01.dta, clear
...
sort id year
gen tenure_emp = 1
replace tenure_emp = tenure_emp[_n-1] + 1 if id == id[_n-1] & switch_emp != 1
gen tenure_occ = 1
replace tenure_occ = tenure_occ[_n-1] + 1 if id == id[_n-1] & switch_occ != 1
gen tenure_ind = 1
replace tenure_ind = tenure_ind[_n-1] + 1 if id == id[_n-1] & switch_ind != 1
by id: gen exp = _n

/*------------------------------------------------------------------------------------*/

/* take out time effects: index wages to average wage */
merge m:1 year using $result/av_wg_cpi.dta
drop if _merge==2
sum cpi if year==2000, meanonly
local cpi_2000 = r(mean)
gen rwage = wage/cpi*`cpi_2000'
drop _merge

/* generate logged valued of wages and earnings */
gen lwage = log(rwage)

/* selection by real wage, if in top 0.1% or in bottom 0.1% or if make less than 2.00 per hour*/
drop if rwage == .
drop if rwage <= 0
_pctile rwage, p(0.1, 99.9)
drop if rwage <= r(r1)
drop if rwage >= r(r2)
*drop if rwage <= 200                                                                                                         /* CHECK LATER */
...
save $result/yearly_02.dta, replace

/* 13change.do 14fix.do in pavan */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
