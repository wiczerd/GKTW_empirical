...
global result = "~/workspace/GKTW_empirical/nlsy79/result/hlthcomp1_std1_subset2"
global data_loc = "~/workspace/GKTW_empirical/nlsy79/result"
set more off
...
// old, young
local cond1 = "(age < 35)"
local collab1 = "Age $< 35$"
local cond2 = "(age >= 35)"
local collab2 = "Age $\geq 35$"
// college, non-college
local cond3 = "(univ)"
local collab3 = "College Grad"
local cond4 = "(!univ)"
local collab4 = "No College"
// everyone
local cond5 = 1
local collab5 = "All"

use $data_loc/yearly_02.dta, clear
...
// this loop does not work for condition  age < 35
/*
gen residwage = .
...
if($no_phys ==1 | $no_mech==1 | $twoskil == 1 ){
	rename mm mm3
...
if($no_mech==1){
	...
	gen ability_m2 = (ability_1 + ability_3)/2
	gen skill_m2  = (skill_1 + skill_3)/2
	egen ability_sd2 = rowsd(ability_1 ability_3)
	egen skill_sd2 = rowsd(skill_1 skill_3)

	if($match_centered==1){
		gen match2 = ( (ability_1 - ability_m2)*(skill_1 - skill_m2) + (ability_3 - ability_m2)*(skill_3 - skill_m2) )/2
	}
	else{
		gen match2 = ( ability_1*skill_1 + ability_3*skill_3 )/2
	}
}
else{
...
if($twoskil == 1){
	...
	drop ability_m2 skill_m2 ability_sd2 skill_sd2 match2
	gen ability_m2 = (ability_0 + ability_3)/2
	gen skill_m2  = (skill_0 + skill_3)/2
	egen ability_sd2 = rowsd(ability_0 ability_3)
	egen skill_sd2 = rowsd(skill_0 skill_3)
	if($match_centered==1){
		gen match2 = ( (ability_0 - ability_m2)*(skill_0 - skill_m2) + (ability_3 - ability_m2)*(skill_3 - skill_m2) )/2
	}
	else{
		gen match2 = ( ability_0*skill_0 + ability_3*skill_3 )/2
	}
}
...
save $data_loc/yearly_03_phys.dta, replace
forvalues num = 1/5 {

use $data_loc/yearly_03_phys.dta, clear
// subsetting happens here
disp "SUBSETTING"
keep if `cond`num''


save $data_loc/yearly_03_phys_subset.dta, replace
...
if `num' == 2{
	local collab = "Age $\geq 35$"
}
// college, non-college
if `num' == 3{
	local collab = "College Grad"
}
if `num' == 4{
	local collab = "No College"
}
// everyone
if `num' == 5{
	local collab = "All"
...
// regression 1
use $data_loc/yearly_03_phys_subset.dta, clear
...
outreg2 using $result/Tables/reg2_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(match match_ten_occ) label tstat
...
outreg2 using $result/Tables/reg2-1_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(mm mm_ten_occ) label tstat
...
outreg2 using $result/Tables/reg4_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(match match_ten_occ cmatch_exp ability_m skill_m ability_m_ten_occ skill_m_ten_occ) label tstat
...
outreg2 using $result/Tables/reg5_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(mm cmm_exp mm_ten_occ ability_m skill_m ability_m_ten_occ skill_m_ten_occ) label tstat
...
outreg2 using $result/Tables/reg6_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(match match_ten_occ ability_m skill_m ability_m_ten_occ skill_m_ten_occ) label tstat
...
outreg2 using $result/Tables/reg7_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(mm mm_ten_occ ability_m skill_m ability_m_ten_occ skill_m_ten_occ) label tstat
...
xi: ivregress 2sls lwage match_1 match_2 match_3 ($xlist = $ivlist) $zlist ability_m skill_m ability_skill_m i.ind_1d i.occ_1d, vce(robust)
outreg2 using $result/Tables/reg8_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(ability_m skill_m ability_m_ten_occ skill_m_ten_occ) label tstat
...
xi: ivregress 2sls lwage absmm_1 absmm_2 absmm_3 ($xlist = $ivlist) $zlist ability_m skill_m ability_skill_m  i.ind_1d i.occ_1d, vce(robust)
*/
...
outreg2 using $result/Tables/reg10_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(ability_m mm_neg mm_pos) label tstat
// regression 11
probit switch_occ mm ability_m age age2 $zlist i.occ_1d  i.ind_1d
outreg2 using $result/Tables/reg11_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(mm ability_m) label tstat
// regression 12
probit switch_occ match ability_m skill_m age age2 $zlist i.occ_1d  i.ind_1d
outreg2 using $result/Tables/reg12_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(match ability_m skill_m) label tstat
// regression 13
/* NOTHING WRONG WITH THESE, JUST SLOW
xtprobit switch_occ mm_neg mm_pos ability_m age age2 $zlist i.occ_1d  i.ind_1d
outreg2 using $result/Tables/reg13_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(ability_m mm_neg mm_pos) label tstat
// regression 14
xtprobit switch_occ mm ability_m age age2 $zlist i.occ_1d  i.ind_1d
outreg2 using $result/Tables/reg14_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(mm ability_m) label tstat
// regression 15
xtprobit switch_occ match ability_m skill_m age age2 $zlist i.occ_1d  i.ind_1d
outreg2 using $result/Tables/reg15_hlth$hlth_comp_use.tex, bdec(3) tex(fragment) ctitle("`collab'", " ") keep(match ability_m skill_m) label tstat
*/
