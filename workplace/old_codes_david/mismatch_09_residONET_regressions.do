/*--------------------------------------------------------------------------------------
* name: mismatch_09_resdONET_regression.do
...
/* create life-cycle residual ONET */
gen age2 =age*age
gen age3 = age2*age
qui forvalues i=1/7{
	reg ONET_ASVAB_`i' age age2 age3 exp exp2 exp3 univ lths
	predict ONET_res_`i' if ONET_ASVAB_`i'<., residuals
	qui sum ONET_ASVAB_`i', meanonly
	replace ONET_res_`i' = ONET_res_`i'+r(mean)
}
...
forvalues i=1/7{
	qui sum ONET_res_`i'
	gen ONET_std`i' = (ONET_res_`i')/r(sd)
	gen ONET_maxscl`i' = ONET_res_`i'/r(max)
	gen ONET_log`i' = log(ONET_res_`i'*6)
	qui sum ONET_log`i'
	gen ONET_stdlog`i' = ONET_log`i'/r(sd)
/*	this is the built-in to make ranks
	cumul ONET_ASVAB_`i' [aw=occ_wt], gen(ONET_rnk_`i') equal */
	gsort -occ_indic + ONET_res_`i'
...
forvalues i=1/7{
	...
	qui sum ONET_res_`i', meanonly
	qui replace sum_ONET = sum_ONET + ONET_res_`i'/r(max)
}
...
if($stdize_match ==1){
	...
	egen ONET_m = rowmean(ONET_res_?)
	egen asvab_sd = rowsd(asvab_res0?)
	egen ONET_sd = rowsd(ONET_res_?)
	gen match = 0
	forvalues i = 1/7 {
		quietly replace match = match + (asvab_res0`i' - asvab_m)*(ONET_res_`i'-ONET_m)/7.
	}
}
	...
	gen match = 0
	forvalues i = 1/7 {
		quietly replace match = match + (asvab_AFQT`i' - asvab_m)*(ONET_res_`i'-ONET_m)/7.
	}
}
...
graph export $result/pred_ONET_res_shape.eps, replace
...
* change name
rename ONET_res_1 AR_ONET
rename ONET_res_2 MK_ONET
rename ONET_res_3 WK_ONET
rename ONET_res_4 PC_ONET
rename ONET_res_5 GS_ONET
rename ONET_res_6 MC_ONET
rename ONET_res_7 EI_ONET
