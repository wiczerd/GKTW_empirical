...
* creates many instruments for BNLTSLS, and uses these instruments
--------------------------------------------------------------------------------------*/
...
/* physical ability */
reg hlth_composite I_enterage1-I_enterage3 I_enterage5-I_enterage9 /*lths univ black hisp*/
...
bysort id: egen age_skh_init_tmp =min(age) if skill_h<.
bysort id: egen age_skh_init = min(age_bmi_init_tmp)
gen skill_h_init_tmp1 = skill_h if age == age_skh_init
bysort id: egen skill_h_init =min(skill_h_init_tmp1)
...
reg ability_h $hlth_list, vce(robust)
/* csv */
...
global hlth_list_cc ${hlth_list} skill_h
global hlth_list_pos_neg ${hlth_list} mm_aa_??? mm_bb_??? ability_m ability_v skill_m skill_v skill_h 
global hlth_list ${hlth_list} ability_m ability_v skill_m skill_v skill_h absmm_aa absmm_bb

/* all of the cross terms */
global hlth_list2 
...
global hlth_list_ten 
global hlth_list_ten_iv 
foreach hv of varlist $hlth_list {
	gen `hv'_tnoc = `hv'*tenure_occ
	global hlth_list_ten ${hlth_list_ten} `hv'_tnoc 
	gen `hv'_tnoc_iv = `hv'*ten_occ_iv
	global hlth_list_ten_iv ${hlth_list_ten_iv} `hv'_tnoc_iv
}
/* cross terms for pos_neg*/
global hlth_list_pos_neg2 
foreach hv of varlist height bmi_init2 bmi_init{
	foreach hvv of varlist $hlth_list_pos_neg{
		capture gen `hv'X`hvv' = `hv'*`hvv'
		global hlth_list_pos_neg2 $hlth_list_pos_neg2 `hv'X`hvv'
	}
}
global hlth_list_pos_neg $hlth_list_pos_neg $hlth_list_pos_neg2
// generate hlth_list_ten
global hlth_list_pos_neg_ten 
global hlth_list_pos_neg_ten_iv 
foreach hv of varlist $hlth_list_pos_neg{
	capture gen `hv'_tnoc = `hv'*tenure_occ
	global hlth_list_ten ${hlth_list_pos_neg_ten} `hv'_tnoc 
	capture gen `hv'_tnoc_iv = `hv'*ten_occ_iv
	global hlth_list_pos_neg_ten_iv ${hlth_list_pos_neg_ten_iv} `hv'_tnoc_iv
}

// cross terms for cc
global hlth_list_cc2 
foreach hv of varlist height bmi_init2 bmi_init{
	foreach hvv of varlist $hlth_list_cc{
		global hlth_list_cc2 $hlth_list_cc2 `hv'X`hvv'
	}
}
global hlth_list_cc $hlth_list_cc $hlth_list_cc2
// generate hlth_list_cc_ten
global hlth_list_cc_ten 
global hlth_list_cc_ten_iv 
foreach hv of varlist $hlth_list_cc {
	global hlth_list_cc_ten ${hlth_list_cc_ten} `hv'_tnoc 
	global hlth_list_cc_ten_iv ${hlth_list_cc_ten_iv} `hv'_tnoc_iv
}
...
/////health iv/////
///////////////////
/* mismatch for health iv */

qui foreach mmv of varlist mm mm_sgn mm_neg mm_pos {
	reg `mmv' ${hlth_list} , vce(robust)
	predict `mmv'_iv
}
qui foreach mmv of varlist mm_cc mm_cc_neg mm_cc_pos absmm_cc ability_cc{
	reg `mmv' ${hlth_list_cc} , vce(robust)
	predict `mmv'_iv
}
...
/*------------------------------------------------------------------------------------*/
* w/o tenure iv
qui foreach mmv of varlist mm mm_pos mm_neg {
	qui reg `mmv'_ten_occ ${hlth_list_pos_neg} ${hlth_list_pos_neg_ten}, vce(robust)
	predict `mmv'_iv_ten_occ
}
qui foreach mmv of varlist absmm_cc mm_cc_neg mm_cc_pos mm_cc ability_cc{
	qui reg `mmv'_ten_occ ${hlth_list_cc} ${hlth_list_cc_ten}, vce(robust)
	predict `mmv'_iv_ten_occ
}
* w/ tenure iv
qui reg mm_ten_occ_iv ${hlth_list} ${hlth_list_ten_iv}, vce(robust)
predict mm_iv_ten_occ_iv
qui foreach mmv of varlist mm_neg mm_pos{
	reg `mmv'_ten_occ_iv ${hlth_list_pos_neg} ${hlth_list_pos_neg_ten_iv}, vce(robust)
	predict `mmv'_iv_ten_occ_iv
}
qui foreach mmv of varlist absmm_cc mm_cc mm_cc_neg mm_cc_pos ability_cc{
	reg `mmv'_ten_occ_iv ${hlth_list_cc} ${hlth_list_cc_ten_iv}, vce(robust)
	predict `mmv'_iv_ten_occ_iv
}
...
foreach i of local nlist{
	...
	gen lskill_`i' = skill_`i'[_n-1] if switch_occ == 1
	replace lskill_`i' = lskill_`i'[_n-1] if switch_occ == 0 & id == id[_n-1]

}

foreach lmmv of varlist lmm lmm_?? lmm_??_neg lmm_??_pos{
	qui reg `lmmv' ${hlth_list}, vce(robust)
	predict `lmmv'_iv	
}
...
foreach i of local nlist{
	...
	by id: gen cskill_`i' = sum(lskill_`i'*lten_occ)/totexp if switch_occ==1  & lmm_`i'<. & lten_occ<.
...
foreach i of local nlist{
	...
	replace cskill_`i' = cskill_`i'[_n-1] if switch_occ == 0 & id == id[_n-1]
...
/* build cmm instruments */
global hlth_list_cmm cmm_aa cmm_bb cskill_??
foreach cmmv of varlist $hlth_list_cmm{
	gen `cmmv'2 = `cmmv'*`cmmv'
	global hlth_list_cmm `cmmv'2
}
/* build cmm pos_neg instruments */
global hlth_list_cmm_pos_neg cmm_???_aa cmm_???_bb cskill_??
foreach cmmv of varlist $hlth_list_cmm_pos_neg {
	capture gen `cmmv'2 = `cmmv'*`cmmv'
	global hlth_list_cmm_pos_neg `cmmv'2
}
/* build cmm_cc instruments */
global hlth_list_cmm_cc lskill_cc cskill_cc
foreach cmmv of varlist $hlth_list_cmm_cc{
	global hlth_list_cmm_cc `cmmv'2
}
reg cmm ${hlth_list} ${hlth_list_cmm_pos_neg} ability_aa ability_bb, vce(robust)
predict cmm_iv

qui foreach cmmv of varlist cmm_neg cmm_pos {
	reg `cmmv' ${hlth_list} ${hlth_list_cmm_pos_neg} ability_aa ability_bb, vce(robust)
	predict `cmmv'_iv
}

qui foreach cmmv of varlist cmm_cc cmm_neg_cc cmm_pos_cc {
	reg `cmmv' ${hlth_list_cc} ${hlth_list_cmm_cc}, vce(robust)
	predict `cmmv'_iv
}
...
/* this one is only tenure iv without health iv */
/*
global xlist mm_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
...
global ivlist mm_iv cmm_iv mm_iv_ten_occ
xi: ivregress 2sls lwage ($xlist = $ivlist) $zlist $xlist_0 ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d, vce(robust)
...
global ivlist mm_neg_iv mm_pos_iv cmm_pos_iv cmm_neg_iv mm_pos_iv_ten_occ mm_neg_iv_ten_occ
xi: ivregress 2sls lwage ($xlist = $ivlist) $zlist $xlist_0 ability_mean skill_mean ability_mean_ten_occ skill_mean_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/olsh_cmm_mm_means_pos_neg.ster, replace

global xlist  mm cmm mm_ten_occ $xlist_0 ability_mean_ten_occ skill_mean_ten_occ
global ivlist mm_iv cmm_iv mm_iv_ten_occ_iv $ivlist_0 ability_mean_ten_occ_iv skill_mean_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/ivh_cmm_mm_means.ster, replace

global xlist  mm_neg mm_pos cmm_pos cmm_neg mm_pos_ten_occ mm_neg_ten_occ $xlist_0 ability_mean_ten_occ skill_mean_ten_occ
global ivlist mm_neg_iv mm_pos_iv cmm_pos_iv cmm_neg_iv mm_pos_iv_ten_occ_iv mm_neg_iv_ten_occ_iv $ivlist_0 ability_mean_ten_occ_iv skill_mean_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(robust)
...
global ivlist absmm_cc_iv absmm_cc_iv_ten_occ
...
global ivlist absmm_cc_iv absmm_aa_ten_occ_iv absmm_bb_ten_occ_iv absmm_cc_iv_ten_occ_iv $ivlist_0 ability_aa_ten_occ_iv ability_bb_ten_occ_iv ability_cc_iv_ten_occ_iv skill_??_ten_occ_iv 
...
global xlist  cmm_cc absmm_cc ability_cc ability_cc_ten_occ absmm_cc_ten_occ
global ivlist cmm_cc_iv absmm_cc_iv absmm_cc_iv_ten_occ ability_cc_iv ability_cc_iv_ten_occ
xi: ivregress 2sls lwage cmm_aa absmm_aa cmm_bb absmm_bb absmm_aa_ten_occ absmm_bb_ten_occ ($xlist = $ivlist) $zlist $xlist_0 ability_aa ability_bb skill_?? ability_aa_ten_occ ability_bb_ten_occ skill_??_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save ${result}/olsh_ind_cmm_mm_means.ster, replace

global xlist  cmm_cc absmm_cc ability_cc absmm_??_ten_occ $xlist_0 ability_??_ten_occ skill_??_ten_occ
global ivlist cmm_cc_iv absmm_cc_iv absmm_cc_iv_ten_occ_iv ability_cc_iv_ten_occ_iv ability_cc_iv /*
		*/ $ivlist_0 absmm_aa_ten_occ_iv absmm_bb_ten_occ_iv ability_aa_ten_occ_iv ability_bb_ten_occ_iv skill_??_ten_occ_iv
xi: ivregress 2sls lwage cmm_aa absmm_aa cmm_bb absmm_bb ($xlist = $ivlist) /*
	*/ $zlist ability_aa ability_bb skill_?? i.ind_1d i.occ_1d, vce(robust)
...
/* csv */
esttab iv_mm_means iv_cmm_mm_means olsh_mm_means olsh_cmm_mm_means ols_mm_means ols_cmm_mm_means ///
                   using ${result}/table_${diminitls}.csv, b(4) ///
                   r2 nodepvars nogaps not label nonotes drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("IV: TH" "IV: TH" "OLS: H" "OLS: H" "OLS" "OLS") ///
...
//////////////////////////////
///// pos-neg table /////                                                                                                                                                                                                                                 /* check later */
//////////////////////////////

esttab iv_cmm_mm_means_pos_neg olsh_cmm_mm_means_pos_neg ols_cmm_mm_means_pos_neg ///
                   using ${result}/table_${diminitls}_pos_neg.tex, b(4) ///
                   r2 nodepvars gaps not label nonotes substitute(\hline\hline \hline \hline "\hline  " "Standard" "Robust standard" ///
                   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("IV: TH" "OLS: H" "OLS") ///
		   order(mm_pos mm_neg mm_pos_ten_occ mm_neg_ten_occ cmm_pos cmm_neg ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ ) ///
                   star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
...
esttab iv_cmm_mm_means_pos_neg olsh_cmm_mm_means_pos_neg ols_cmm_mm_means_pos_neg ///
		   ...
		   drop(_I*) ///
		   mtitles("IV: TH" "OLS: H" "OLS") ///
		   order(mm_pos mm_neg mm_pos_ten_occ mm_neg_ten_occ cmm_pos cmm_neg ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ ten* exp* oj $zlist _cons) ///
                   star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
		   


/*------------------------------------------------------------------------------------*/
/* tables for component regression */
...
/* csv */
esttab iv_ind_mm_means iv_ind_cmm_mm_means olsh_ind_mm_means olsh_ind_cmm_mm_means ols_ind_mm_means ols_ind_cmm_mm_means ///
...
estimate save ${result}/olsh_lpm_${diminitls}_mm.ster, replace

xi: ivregress 2sls fswitch_occ absmm_aa absmm_bb (absmm_cc  = absmm_h_iv) $prswlist, vce(robust)
estimate save ${result}/olsh_lpm_${diminitls}_mm_ind.ster, replace
...
xi: ivregress 2sls fswitch_occ absmm_aa absmm_bb ($prswlist_x absmm_cc  = ${prswlist_iv} absmm_cc_iv) $prswlist_z, vce(robust)
estimate save ${result}/ivh_lpm_${diminitls}_mm_ind.ster, replace

/*------------------------------------------------------------------------------------*/

//////////////////////////////
///// additional stuff ///////                                                                                                                                                                                               /* check later */
//////////////////////////////

xi: reg fswitch_occ mm_neg mm_pos $prswlist 
estimate save ${result}/lpm_${diminitls}_mm_sgn.ster, replace

xi: ivregress 2sls fswitch_occ (mm_neg mm_pos = mm_neg_iv mm_pos_iv) $prswlist, vce(robust)
estimate save ${result}/olsh_lpm_${diminitls}_mm_sgn.ster, replace

xi: ivregress 2sls fswitch_occ (${prswlist_x} mm_neg mm_pos = ${prswlist_iv} mm_neg_iv mm_pos_iv) $prswlist_z , vce(robust)
estimate save ${result}/ivh_lpm_${diminitls}_mm_sgn.ster, replace
...
estimate use ${result}/olsh_lpm_${diminitls}_mm.ster
estimate store olsh_lpm_${diminitls}_mm
estimate use ${result}/olsh_lpm_${diminitls}_mm_ind.ster
estimate store olsh_lpm_${diminitls}_mm_ind
...
estimate store lpm_${diminitls}_mm_sgn
estimate use ${result}/olsh_lpm_${diminitls}_mm_sgn.ster
estimate store olsh_lpm_${diminitls}_mm_sgn
estimate use ${result}/ivh_lpm_${diminitls}_mm_sgn.ster
estimate store ivh_lpm_${diminitls}_mm_sgn
/*
...
*/
/* csv */
esttab ivh_lpm_${diminitls}_mm ivh_lpm_${diminitls}_mm_ind olsh_lpm_${diminitls}_mm olsh_lpm_${diminitls}_mm_ind lpm_${diminitls}_mm lpm_${diminitls}_mm_ind ///
...
//////////////////////////////
///// additional table 2 /////                                                                                                                                                                                                                                 /* check later */
//////////////////////////////
esttab ivh_lpm_${diminitls}_mm_sgn olsh_lpm_${diminitls}_mm_sgn lpm_${diminitls}_mm_sgn ///
                   using ${result}/table_${diminitls}_switch_lpm_sgn.tex, b(4) se(4) ///
                   nodepvars nogaps label longtable substitute(\hline\hline \hline \hline "\noalign{\vskip 1mm} \hline \noalign{\vskip 1mm}" ///
		   "Standard errors in parentheses" "All regressions include occupation and industry dummies." ///
		   "\sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)" "Robust standard errors in parentheses. \sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)." ///
                   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   mtitles("LPM: TH" "LPM: H" "LPM") ///
		   title("Regressions for Probability of Occupational Switch with Signed Mismatch") ///
		   order(mm_pos mm_neg ability_mean ability_mean* skill_mean skill_mean*) drop(_cons _I* tenure_occ ten_occ2 tenure_emp ten_emp2 exp exp2 oj $zlist) ///
                   star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
