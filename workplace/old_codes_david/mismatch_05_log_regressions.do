/*--------------------------------------------------------------------------------------
* name: mismatch_05_log_regression.do
...
gen lmatch_exp = lmatch*exp
gen cmatch_exp = cmatch*exp

label var lmm "Last Mismatch"
label var cmm "Cumulative Mismatch"
label var lmm_exp "Last Mismatch$\times$ Prior Exp"
label var cmm_exp "Cumulative Mismatch$\times$ Prior Exp"

label var lmatch "Last Match Quality"
label var cmatch "Cumulative Match Quality"
label var lmatch_exp "Last Match Quality$\times$ Prior Exp"
label var cmatch_exp "Cumulative Match Quality$\times$ Prior Exp"
...
save $result/yearly_03_log.dta, replace
...
estimate save $result/bench_logols.ster, replace
...
estimate save $result/logols.ster, replace
outreg using $result/table_ols.tex, tex merge bdec(3) ctitle("", "(1)") fragment drop(_I* $zlist)  varlabels se replace
...
estimate save $result/logols_asvab.ster, replace
...
estimate save $result/logols_asvab_occ.ster, replace
outreg, tex merge bdec(3) ctitle("", "(2)") drop(_I* $zlist ) varlabels fragment se replace
...
estimate save $result/logols_ONET.ster, replace
*outreg, tex merge bdec(3) ctitle("", "(4)") drop(_I* $zlist) varlabels fragment se replace
...
estimate save $result/logols_ONET_occ.ster, replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2  cmatch cmatch_exp $zlist sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/logols_cmatch.ster, replace
outreg using $result/table_logols.tex, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace
...
estimate use $result/bench_logols.ster
...
estimate save $result/logols_mm.ster, replace
...
estimate save $result/logols_asvab_mm.ster, replace
*outreg , tex merge bdec(3) ctitle("", "(2)") drop(_I* $zlist ) fragment varlabels se replace
...
estimate save $result/logols_asvab_occ_mm.ster, replace
outreg , tex merge bdec(3) ctitle("", "(2)") drop(_I* $zlist ) fragment varlabels se replace
...
estimate save $result/logols_ONET_mm.ster, replace
*outreg , tex merge bdec(3) ctitle("", "(4)") drop(_I* $zlist ) fragment varlabels se replace
...
estimate save $result/logols_ONET_occ_mm.ster, replace
outreg, tex merge bdec(3) fragment ctitle("", "(3)") drop(_I* $zlist ) varlabels se replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist mm mm_ten_occ mm_ten_occ2  cmm cmm_exp $zlist sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/logols_cmm.ster, replace
outreg using $result/table_logols_mm.tex, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace
...
estimate save $result/bench_logiv.ster, replace
...
estimate save $result/logiv.ster, replace
...
estimate save $result/logiv_asvab.ster, replace
*outreg, tex merge bdec(3) fragment ctitle("", "(2)") drop(_I* $zlist ) varlabels se replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_asvab_occ.ster, replace
outreg, tex merge bdec(3) fragment ctitle("", "(2)") drop(_I* $zlist ) varlabels se replace
...
estimate save $result/logiv_ONET.ster, replace
*outreg, tex merge bdec(3) fragment ctitle("", "(4)") drop(_I* $zlist ) varlabels se replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist sum_ONET_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_ONET_occ.ster, replace
outreg using, tex merge bdec(3) fragment ctitle("", "(3)") drop(_I* $zlist ) varlabels se replace


/* IV regression with match + cmatch + ONET +ASVAB */
...
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv match_ten_occ_iv match_ten_occ2_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) match cmatch cmatch_exp $zlist sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_cmatch.ster, replace
outreg using $result/table_logiv.tex, tex merge bdec(3) fragment ctitle("", "(4)") drop(_I* $zlist ) varlabels se replace
...
estimate use $result/bench_logiv.ster
...
estimate save $result/logiv_mm.ster, replace
...
estimate save $result/logiv_asvab_mm.ster, replace
*outreg, tex merge bdec(3) fragment ctitle("", "(2)") drop(_I* $zlist ) varlabels se replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) mm $zlist sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_asvab_occ_mm.ster, replace
outreg, tex merge bdec(3) fragment ctitle("", "(2)") drop(_I* $zlist ) varlabels se replace
...
estimate save $result/logiv_ONET_mm.ster, replace
*outreg, tex merge bdec(3) fragment ctitle("", "(4)") drop(_I* $zlist ) varlabels se replace
...
xi: ivregress 2sls lwage ($xlist = $ivlist) mm $zlist sum_ONET_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_ONET_occ_mm.ster, replace
outreg using $result/table_logiv_mm.tex, tex merge bdec(3) fragment ctitle("", "(3)") drop(_I* $zlist ) varlabels se replace

/* IV regression with mismatch + cumulative mismatch + ONET + ASVAB */
...
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj  mm_ten_occ mm_ten_occ2
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv mm_ten_occ_iv mm_ten_occ2_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) mm cmm cmm_exp $zlist sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/logiv_cmm_mm.ster, replace
outreg using $result/table_logiv_mm.tex, tex merge bdec(3) fragment ctitle("", "(4)") drop(_I* $zlist ) varlabels se replace
...
clear
estimate use $result/logols.ster
matrix eb=e(b)
...
saving($result/marg_effect_logols, replace)
graph export $result/marg_effect_logols.eps, replace
drop marg_match*

estimate use $result/logiv.ster
matrix eb=e(b)
matrix marg_match = J(16,4,0.0)
forvalues tt=0/15{
	matrix marg_match[`tt'+1,1] = `tt'
...
lpattern("_" "-" "_-" ) ///
legend(label(1 "+1 Standard Dev") label(2 "Mean") label(3 "-1 Standard Deviation") ) ///
saving($result/marg_effect_logiv, replace)
graph export $result/marg_effect_logiv.eps, replace
drop marg_match*

graph combine $result/marg_effect_logols.gph $result/marg_effect_logiv.gph, ycommon
graph export $result/marg_effect_logols_logiv.eps, replace

clear
estimate use $result/logols_mm.ster
matrix eb=e(b)
matrix marg_match = J(16,4,0.0)
forvalues tt=0/15{
	matrix marg_match[`tt'+1,1] = `tt'
...
lpattern("_" "-" "_-" ) ///
legend(label(1 "+1 Standard Dev") label(2 "Mean") label(3 "-1 Standard Deviation") ) ///
saving($result/marg_effect_logols_mm, replace)
graph export $result/marg_effect_logols_mm.eps, replace
drop marg_match*

estimate use $result/logiv_mm.ster
matrix eb=e(b)
matrix marg_match = J(16,4,0.0)
forvalues tt=0/15{
	matrix marg_match[`tt'+1,1] = `tt'
...
lpattern("_" "-" "_-" ) ///
legend(label(1 "+1 Standard Dev") label(2 "Mean") label(3 "-1 Standard Deviation") ) ///
saving($result/marg_effect_logiv_mm, replace)
graph export $result/marg_effect_logiv_mm.eps, replace
drop marg_match*

graph combine $result/marg_effect_logols_mm.gph $result/marg_effect_logiv_mm.gph, ycommon
graph export $result/marg_effect_logols_logiv_mm.eps, replace
...
saving($result/pred_effect_logols, replace)
graph export $result/pred_effect_logols.eps, replace
drop pred_match*
...
lpattern("l" "_" "_-" "_--" "--") ///
legend(label(1 "Top 10%") label(2 "Top 30%") label(3 "Top 50%") label(4 "Top 70%") label(5 "Top 90%")) ///
saving($result/pred_effect_logiv, replace)
graph export $result/pred_effect_logiv.eps, replace
drop pred_match*

graph combine $result/pred_effect_logols.gph $result/pred_effect_logiv.gph, ycommon
graph export $result/pred_effect_logols_logiv.eps, replace
...
forvalues tt=0/15{
	matrix pred_match[`tt'+1,1] = `tt'
        matrix pred_match[`tt'+1,2] = _b["mm"]*`p90'+_b["mm_ten_occ"]*`p90'*`tt' +_b["mm_ten_occ2"]*`p90'*`tt'^2/100
        matrix pred_match[`tt'+1,3] = _b["mm"]*`p70'+_b["mm_ten_occ"]*`p70'*`tt' +_b["mm_ten_occ2"]*`p70'*`tt'^2/100
        matrix pred_match[`tt'+1,4] = _b["mm"]*`p50'+_b["mm_ten_occ"]*`p50'`tt' +_b["mm_ten_occ2"]*`p50'*`tt'^2/100
        matrix pred_match[`tt'+1,5] = _b["mm"]*`p30'+_b["mm_ten_occ"]*`p30'*`tt' +_b["mm_ten_occ2"]*`p30'*`tt'^2/100
        matrix pred_match[`tt'+1,6] = _b["mm"]*`p10'+_b["mm_ten_occ"]*`p10'*`tt' +_b["mm_ten_occ2"]*`p10'*`tt'^2/100
}
svmat pred_match
twoway line pred_match2-pred_match6 pred_match1, ///
ytitle("Predicted Return") ///
...
lpattern("l" "_" "_-" "_--" "--") ///
legend(label(1 "Top 10%") label(2 "Top 30%") label(3 "Top 50%") label(4 "Top 70%") label(5 "Top 90%")) ///
saving($result/pred_effect_logols_mm, replace)
graph export $result/pred_effect_logols_mm.eps, replace
drop pred_match*
...
lpattern("l" "_" "_-" "_--" "--") ///
legend(label(1 "Top 10%") label(2 "Top 30%") label(3 "Top 50%") label(4 "Top 70%") label(5 "Top 90%")) ///
saving($result/pred_effect_logiv_mm, replace)
graph export $result/pred_effect_logiv_mm.eps, replace
drop pred_match*

graph combine $result/pred_effect_logols_mm.gph $result/pred_effect_logiv_mm.gph, ycommon
graph export $result/pred_effect_logols_logiv_mm.eps, replace
...
estimate save $result/logxt_nsw.ster, replace
...
estimate save $result/logols_nsw.ster, replace
outreg , tex merge bdec(3) ctitle("", "(2)") drop(_I*) varlabels se replace  fragment

gen delmatch = match - l.match
xi: reg delmatch  nswitch $zlist i.ind_1d i.occ_1d  if nswitch>0 & nswitch<6 , vce(robust)
estimate save $result/del_nsw_logmm.ster, replace
...
estimate save $result/logiv_exp.ster, replace
...
estimate save $result/logxt_exp.ster, replace
outreg , tex merge bdec(3) ctitle("", "(5)") drop(_I* ) varlabels se replace fragment

xi: reg match  $xlist  $zlist i.ind_1d i.occ_1d
estimate save $result/logols_exp.ster, replace
outreg using $result/table_logmatch.tex, tex merge bdec(3) ctitle("", "(6)") drop(_I* ) varlabels se replace fragment
...
estimate save $result/logxt_nsw_mm.ster, replace
outreg, tex bdec(3) ctitle("", "(1)") drop(_I*) varlabels se replace fragment
...
estimate save $result/logols_nsw_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(2)") drop(_I*) varlabels se replace fragment

gen delmm = mm - l.mm
xi: reg delmm nswitch $zlist i.ind_1d i.occ_1d  if nswitch>0 & nswitch<6 & switch_occ==1, vce(robust)
estimate save $result/del_nsw_logmm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(3)") drop(_I*) varlabels se replace fragment
...
estimate save $result/logiv_exp_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(3)") drop(_I* ) varlabels se replace fragment

xi: xtreg mm $xlist $zlist i.ind_1d i.occ_1d, vce(robust)
estimate save $result/logxt_exp_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(4)") drop(_I* ) varlabels se replace fragment

xi: reg mm $xlist $zlist i.ind_1d i.occ_1d
estimate save $result/logiv_exp_mm.ster, replace
outreg using $result/table_logmm.tex, tex merge bdec(3) ctitle("", "(5)") drop(_I* ) varlabels se replace fragment
...
xlabel(20(5)50) ///
saving($result/logmatch_by_age, replace)
graph export $result/logmatch_by_age.eps, replace
...
xlabel(5(5)30) ///
saving($result/logmatch_labexp, replace)
graph export $result/logmatch_labexp.eps, replace
...
saving($result/logmatch_by_sw, replace)
graph export $result/logmatch_by_sw.eps, replace

/* average match quality by labor market experience */
lpoly mm exp if exp <= 30, ci noscatter ///
ytitle("Mismatch Measure") ///
xtitle("Labor Market Experience") ///
title("Mismatch by Labor Market Experience") ///
xlabel(5(5)30) ///
saving($result/logmismatch_labexp, replace)
graph export $result/logmismatch_labexp.eps, replace

/* average mismatch by number of switches */
//bysort nswitch: egen mm_sw= mean(mm_log1)
lpoly mm nswitch if age >= 18 & age <= 50 & nswitch <= 5, ci noscatter ///
ytitle("Mismatch Measure") ///
xtitle("Number of Switches") ///
title("Mismatch by the Number of Switches") ///
saving($result/logmismatch_by_sw, replace)
graph export $result/logmismatch_by_sw.eps, replace
