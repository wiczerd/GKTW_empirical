...
forvalues i=1/7{
	qui reg asvab_sec0`i' I_enterage1-I_enterage4 I_enterage6-I_enterage9 /* I_enterage1-I_enterage8 */
...
if $rm_age == 2{
	...
	qui forvalues i=1/7{
		sum asvab_res0`i' if tmp2== 20
		local sd20 = r(sd)
		forvalues ai=16/24{
			sum asvab_res0`i' if tmp2 ==`ai'
			replace asvab_res0`i' = asvab_res0`i'/r(sd)*`sd20' if tmp2 ==`ai'
		}
	}
...
local NKDE = 100
if $rm_age == 3{
	/* clean out age effects from the entire distribution*/
	levelsof tmp2, local(ages) 
	qui forvalues i =1/7{
		gen asvab_tmp = .
		akdensity asvab_res0`i' if tmp2==20, cdf(asvab_q) gen(asvab_x asvab_den) nograph n(`NKDE ')
		mkmat asvab_q if asvab_q<.
		mkmat asvab_x if asvab_x<.
		sort tmp2 asvab_x 
		gen _n_tmp2 = _n
		foreach ai of local ages{
			sum _n_tmp2 if tmp2 == `ai', meanonly
			local t0 = r(min)
			local tT = r(max)
			replace asvab_tmp = . in `t0'/`tT'
			forvalues j = 1/`NKDE '{
				local asvab_qj1 = asvab_q[`j',1]
				disp `asvab_qj1'
				gen dist_tmp = abs(asvab_agernk`i' - `asvab_qj1') if tmp2 == `ai'
				local asvab_xj1 = asvab_x[`j',1]
				replace asvab_tmp = `asvab_xj1' if dist_tmp < 1/`NKDE'*1/2  & tmp2==`ai'
				drop dist_tmp
			}
			sort tmp2 asvab_agernk`i'
			**syntax: ipolate yvar xvar , generate(newvar) epolate 
			ipolate asvab_tmp asvab_agernk`i' if tmp2==`ai', gen(asvab_noage`i') epolate
			replace asvab_res0`i' = asvab_noage`i' if tmp2==`ai'
			drop asvab_noage`i'
		}
		drop asvab_tmp asvab_x asvab_den asvab_q _n_tmp2
	}
}
