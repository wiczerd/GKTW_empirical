...
* instead of leaving each element independent, it groups verbal, math and sci/tech onet and asvab as per the "technical chapter"
--------------------------------------------------------------------------------------*/
...
rename asvab_sec05 asvab_numopp
rename asvab_sec06 asvab_coding
rename asvab_sec07 asvab_auto

rename asvab_sec05_std asvab_numopp_std
rename asvab_sec06_std asvab_coding_std
rename asvab_sec07_std asvab_auto_std
...
qui forvalues i=1/7{
	reg asvab_sec0`i' I_enterage1-I_enterage4 I_enterage6-I_enterage9 /* I_enterage1-I_enterage8 */
	predict asvab_res0`i', residual 
	replace asvab_res0`i' =asvab_res0`i' +_b["_cons"]
}

/* Combine into 3 groups using the principal components as weights*/
pca asvab_res01 asvab_res02, comp(1)
predict asvab_comp01, score /* this composite is the math scores */
qui sum asvab_comp01
replace asvab_comp01 = (asvab_comp01 - r(min) )/r(sd)

pca asvab_res03 asvab_res04, comp(1)
predict asvab_comp02, score /* this composite is the verbal scores */
qui sum asvab_comp02
replace asvab_comp02 = (asvab_comp02 - r(min) )/r(sd)

pca asvab_res05 asvab_res06 asvab_res07, comp(1)
predict asvab_comp03, score /* this composite is the verbal scores */
qui sum asvab_comp03
replace asvab_comp03 = (asvab_comp03 - r(min) )/r(sd)
...
levelsof tmp2, local(ages) 
quietly forvalues i=1/3{
	gen asvab_agernk`i' = .
	foreach ai of local ages{
		cumul asvab_res0`i' [aw=swt] if tmp2==`ai', gen(asvab_agernk) equal
		replace asvab_agernk`i' = asvab_agernk if tmp2==`ai'
		drop asvab_agernk
	}
}
...
forvalues i=1/3{
	qui sum asvab_comp0`i'
	qui gen asvab_std0`i' = (asvab_comp0`i' -r(min))/r(sd)
	qui gen asvab_maxscl0`i' = asvab_comp0`i' /r(max)
	qui gen asvab_log0`i' = log(asvab_comp0`i')
...
gen sum_asvab = 0
forvalues i = 1/3 {
	qui sum asvab_comp0`i', meanonly
        quietly replace sum_asvab = sum_asvab + asvab_comp0`i'/r(max)
}
...
/* create ONET components */
/* Combine into 3 groups using the principal components as weights*/
pca ONET_ASVAB_1 ONET_ASVAB_2, comp(1)
predict ONET_comp1, score /* this composite is the math scores */
qui sum ONET_comp1
replace ONET_comp1 = (ONET_comp1 - r(min) )/r(sd)

pca ONET_ASVAB_3 ONET_ASVAB_4, comp(1)
predict ONET_comp2, score /* this composite is the verbal scores */
qui sum ONET_comp2
replace ONET_comp2 = (ONET_comp2 - r(min) )/r(sd)

pca ONET_ASVAB_5 ONET_ASVAB_6 ONET_ASVAB_7, comp(1)
predict ONET_comp3, score /* this composite is the verbal scores */
qui sum ONET_comp3
...
forvalues i=1/3{
	qui sum ONET_comp`i'
	gen ONET_std`i' = (ONET_comp`i')/r(sd) - r(min)/r(sd)
	gen ONET_maxscl`i' = ONET_comp`i'/r(max)
	gen ONET_log`i' = log(ONET_comp`i'*6)
	qui sum ONET_log`i'
...
gen sum_ONET = 0
forvalues i=1/3{
	qui sum ONET_comp`i', meanonly
	qui replace sum_ONET = sum_ONET + ONET_comp`i'/r(max)
}
...
forvalues i =1/3{
	...
	gen ONET_tmp = .
	forvalues j = 1/`NKDE '{
		qui gen dist_tmp = abs(ONET_rnk`i' - AFQT_pctl[`j'])
		qui replace ONET_tmp = AFQT_x[`j'] if dist_tmp < 1/`NKDE'*1/10
		drop dist_tmp
	}
	sort asvab_rnk`i'
	**syntax: ipolate yvar xvar , generate(newvar) epolate 
	ipolate ONET_tmp ONET_rnk`i', gen(ONET_AFQT`i') epolate
	drop ONET_tmp
}
...
if($stdize_match ==1){
	...
	egen asvab_m = rowmean(asvab_comp0?)
	egen ONET_m = rowmean(ONET_comp?)
	egen asvab_sd = rowsd(asvab_comp0?)
	egen ONET_sd = rowsd(ONET_comp?)
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_comp0`i' - asvab_m)*(ONET_comp`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3{
		quietly replace match = match + (asvab_log0`i' - asvab_m)*(ONET_log`i'-ONET_m)/3.
	}
}
else if($stdize_match ==3){
	/*  USE RANK SCORES HERE: 	*/
	egen asvab_m = rowmean(asvab_rnk?)
	egen ONET_m = rowmean(ONET_rnk?)
	egen asvab_sd = rowsd(asvab_rnk?)
	egen ONET_sd = rowsd(ONET_rnk?)
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_rnk`i' - asvab_m)*(ONET_rnk`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_AFQT`i' - asvab_m)*(ONET_AFQT`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_AFQT`i' - asvab_m)*(ONET_ASVAB_`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_std0`i' - asvab_m)*(ONET_std`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_stdlog0`i' - asvab_m)*(ONET_stdlog`i'-ONET_m)/3.
	}
}
	...
	gen match = 0
	forvalues i = 1/3 {
		quietly replace match = match + (asvab_maxscl0`i' - asvab_m)*(ONET_maxscl`i'-ONET_m)/3.
	}
}
...
forvalues i=1/3{
	replace dsw_ONET_rnk =1/7*abs(ONET_rnk`i' - l.ONET_rnk`i') + dsw_ONET_rnk if switch_occ == 1
}
...
forvalues i=1/3{
	quietly gen rnk_mismatch`i' =    (asvab_rnk`i'- ONET_rnk`i')
	quietly gen abs_mismatch`i' = abs(asvab_rnk`i'- ONET_rnk`i')
}
...
/*------------------------------------------------------------------------------*/
/*-----------------Interaction and distance terms-------------------------------*/
/*------------------------------------------------------------------------------*/

gen mm_01 = abs(asvab_rnk1 - ONET_rnk1)
gen mm_02 = abs(asvab_rnk2 - ONET_rnk2)
gen mm_03 = abs(asvab_rnk3 - ONET_rnk3)

gen mm_01_ten_occ = mm_01*tenure_occ
gen mm_02_ten_occ = mm_02*tenure_occ
gen mm_03_ten_occ = mm_03*tenure_occ

gen mm_01_ten_occ_iv = mm_01*ten_occ_iv
gen mm_02_ten_occ_iv = mm_02*ten_occ_iv
gen mm_03_ten_occ_iv = mm_03*ten_occ_iv

gen match_01 = (asvab_comp01 - asvab_m)* (ONET_comp1 - ONET_m)
gen match_02 = (asvab_comp02 - asvab_m)* (ONET_comp2 - ONET_m)
gen match_03 = (asvab_comp03 - asvab_m)* (ONET_comp3 - ONET_m)

gen match_01_ten_occ = match_01*tenure_occ
gen match_02_ten_occ = match_02*tenure_occ
gen match_03_ten_occ = match_03*tenure_occ

gen match_01_ten_occ_iv = match_01*ten_occ_iv
gen match_02_ten_occ_iv = match_02*ten_occ_iv
gen match_03_ten_occ_iv = match_03*ten_occ_iv

xtset id year
forvalues i=1/3{
	gen lmm_0`i' = l.mm_0`i' if id == l.id & switch_occ==1
	replace lmm_0`i' = l.lmm_0`i' if switch_occ==0
	gen lmatch_0`i' = l.match_0`i' if id == l.id & switch_occ==1
	replace lmatch_0`i' = l.lmatch_0`i' if switch_occ==0
}

gsort +id -switch_occ +year
forvalues i=1/3{
	by id: gen cmatch_0`i' = sum(lmatch_0`i'*lexp) if switch_occ==1  
	by id: gen cmm_0`i' = sum(lmm_0`i'*lexp)  if switch_occ==1 
	replace cmatch_0`i' = cmatch_0`i'/totexp if switch_occ==1 
	replace cmm_0`i' = cmm_0`i'/totexp if switch_occ==1 
}


/*-----------------------------------------------------------------------------*/
/*-------------------Speed tests - attentiveness-------------------------------*/
/*-----------------------------------------------------------------------------*/
if $rm_age == 2{
	/* make the standard deviation the same?*/
	qui foreach asi of varlist asvab_numopp asvab_coding {
		sum `asi' if tmp2== 20
		local sd20 = r(sd)
		forvalues ai=16/24{
			sum `asi' if tmp2 ==`ai'
			replace `asi' = `asi'/r(sd)*`sd20' if tmp2 ==`ai'
		}
	}
}

qui foreach asi of varlist asvab_numopp asvab_coding {
	reg `asi' I_enterage1-I_enterage4 I_enterage6-I_enterage9 /* I_enterage1-I_enterage8 */
	predict `asi'_res, residual 
	replace `asi'_res =`asi'_res +_b["_cons"]
}

pca asvab_numopp_res asvab_coding_res, comp(1)
predict asvab_comp04, score /* this composite is the verbal scores */
qui sum asvab_comp04
replace asvab_comp04 = (asvab_comp04 - r(min) )/r(sd)

pca ONET_15-ONET_21 ONET_80-ONET_81, comp(1)
predict ONET_comp4, score /* this composite is the verbal scores */
qui sum ONET_comp4
replace ONET_comp4 = (ONET_comp4 - r(min) )/r(sd)


/*-----------------------------------------------------------------------------*/
/*-------------------A few regressions to test things--------------------------*/
/*-----------------------------------------------------------------------------*/


global xlist match_01_ten_occ  match_02_ten_occ  match_03_ten_occ  ASVAB_ONET_X_ten_occ asvab_m_ten_occ ONET_m_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  match_01_ten_occ_iv match_02_ten_occ_iv match_03_ten_occ_iv ASVAB_ONET_X_ten_occ_iv asvab_m_ten_occ_iv ONET_m_ten_occ_iv
xi: ivregress 2sls lwage match_01 match_02 match_03 cmatch_exp ASVAB_ONET_X asvab_m ONET_m  ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)


global xlist mm_01_ten_occ  mm_02_ten_occ  mm_03_ten_occ  ASVAB_ONET_X_ten_occ asvab_m_ten_occ ONET_m_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  mm_01_ten_occ_iv mm_02_ten_occ_iv mm_03_ten_occ_iv ASVAB_ONET_X_ten_occ_iv asvab_m_ten_occ_iv ONET_m_ten_occ_iv
xi: ivregress 2sls lwage mm_01 mm_02 mm_03 cmm_exp ASVAB_ONET_X asvab_m ONET_m  ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)
...
corr ONET_rnk?
matrix corrONET = r(C)
matrix colnames corrONET = Math Verbal Tech
matrix rownames corrONET = Math Verbal Tech
...
matrix colnames corrASVAB = Math Verbal Tech
matrix rownames corrASVAB = Math Verbal Tech
...
save $result/yearly_03_3corr.dta, replace
...
use $result/yearly_03_3corr.dta, clear
...
global xlist  tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv    
xi: ivregress 2sls lwage asvab_m asvab_m2 ONET_m ONET_m2 ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)
...
forvalues qi = 1/9{
	...
	matrix pred_ONET_asvab_profile[`qi',2] = `b_onet'*r(r`qi') + `b_onet2'*r(r`qi')^2
}
_pctile asvab_m, p(10 20 30 40 50 60 70 80 90)
forvalues qi = 1/9{
	matrix pred_ONET_asvab_profile[`qi',3] = `b_asvab'*r(r`qi') + `b_asvab2'*r(r`qi')^2
}
clear
...
legend(label(1 "O*NET") label(2 "ASVAB") )
graph export $result/pred_ONET_ASVAB_shape.eps, replace
...
global xlist mm_ten_occ  ASVAB_ONET_X_ten_occ asvab_m_ten_occ ONET_m_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist mm_ten_occ_iv ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  ASVAB_ONET_X_ten_occ_iv asvab_m_ten_occ_iv ONET_m_ten_occ_iv
xi: ivregress 2sls lwage cmm_pos_exp cmm_neg_exp mm asvab_m ONET_m ASVAB_ONET_X ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)

global xlist mm_pos_ten_occ  mm_neg_ten_occ  ASVAB_ONET_X_ten_occ asvab_m_ten_occ ONET_m_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist mm_pos_ten_occ_iv mm_neg_ten_occ_iv  ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  ASVAB_ONET_X_ten_occ_iv asvab_m_ten_occ_iv ONET_m_ten_occ_iv
xi: ivregress 2sls lwage cmm_pos_exp cmm_neg_exp mm_pos mm_neg asvab_m ONET_m ASVAB_ONET_X ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)

global xlist mm_pos_ten_occ  mm_neg_ten_occ  ASVAB_ONET_X_ten_occ asvab_m_ten_occ ONET_m_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist mm_pos_ten_occ_iv mm_neg_ten_occ_iv  ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  ASVAB_ONET_X_ten_occ_iv asvab_m_ten_occ_iv ONET_m_ten_occ_iv
xi: ivregress 2sls lwage mm_pos mm_neg asvab_m ONET_m ASVAB_ONET_X ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)
...
twoway (line pred_cmatch2-pred_cmatch4 pred_cmatch1, lpattern("--" "--" "--") lcolor("red" "black" "blue")  lwidth(thick thick thick) ) ///
(line pred_match2-pred_match4 pred_match1, lpattern("l" "l" "l") lcolor("red" "black" "blue")  lwidth(thick thick thick) ), ///
...
twoway (line pred_cmm2-pred_cmm4 pred_cmm1, lpattern("--" "--" "--") lcolor("red" "black" "blue")  lwidth(thick thick thick) ) ///
(line pred_mm2-pred_mm4 pred_mm1, lpattern("l" "l" "l") lcolor("red" "black" "blue") lwidth(thick thick thick) ), ///
ytitle("Predicted Return") ///
xtitle("Occupational Tenure") ///
title("Predicted Profile By Mismatch: IV") ///
legend(label(1 "90 pctl, cumulative") label(2 "50 pctl") label(3 "10 pctl") ///
...
bysort age: egen med_mm = median(mm)
qui tab age if age>=18 & age<=55, gen(_I_age)
bysort age: egen med_match = median(match)
gen yob = year- age
replace yob = . if yob <1956
qui tab yob, gen(_I_yob)
...
twoway lpoly pr_sw_noedu age if age >= 20 & age <= 50, /// ci
ytitle("Probability of a Switch") ///
xtitle("Age")  lwidth(thick) ///
...
twoway lpoly switch_occ age if age >= 20 & age <= 50, /// ci
ytitle("Probability of a Switch") ///
xtitle("Age") ///
title("Occupational Switches Over the Life-Cycle") ///
xlabel(20(5)50)  lwidth(thick) ///
ylabel(0.05(0.05)0.3) legend(off) ///
...
twoway (lpoly switch_occ age, yaxis(1) ylabel(0.05(0.05)0.3) ytitle("Probability of a Switch", axis(1)) lwidth(thick)) ///
(lpoly match age, yaxis(2) ytitle("Match Quality Measure" , axis(2)) lwidth(thick)) if age >= 20 & age <= 50, ///
xtitle("Age") ///
title("Occupational Switches Over the Life-Cycle") ///
xlabel(20(5)50) legend(label(1 "Pr[Switch]") label(2 "Match")) /// legend(off) 
...
lpoly match age if age >= 20 & age <= 50, ci noscatter ///
ytitle("Match Quality Measure") ///
xtitle("Age")  lwidth(thick) ///
title("Match Quality Over the Life-Cycle") ///
xlabel(20(5)50) legend(off) ///
saving($result/match_by_age, replace)
graph export $result/match_by_age.eps, replace

twoway lpoly match age if age >= 18 & age <= 50, /// ci
ytitle("Match Quality Measure") ///
xtitle("Age") lwidth(thick) ///
title("Match Quality Over the Life-Cycle") ///
xlabel(20(5)50) legend(off) ///
saving($result/match_by_age_noci, replace)
graph export $result/match_by_age_noci.eps, replace
...
twoway (lpoly switch_occ age, yaxis(1) ylabel(0.05(0.05)0.3) ytitle("Probability of a Switch", axis(1)) lwidth(thick) ) ///
(lpoly mm age, yaxis(2) ytitle("Mismatch", axis(2)) lwidth(thick) ) if age >= 20 & age <= 50, ///
xtitle("Age") ///
title("Occupational Switches Over the Life-Cycle") ///
xlabel(20(5)50) legend(label(1 "Pr[Switch]") label(2 "Mismatch"))  /// legend(off) 
saving($result/p_mm_switch_by_age, replace)
graph export $result/p_mm_switch_by_age.eps, replace


/* average mismatch by age */
lpoly mm age if age <= 50 & age >=20, noscatter ci /// 
ytitle("Mismatch Measure") xtitle("Age") ///
title("Mismatch Over the Life-Cycle") ///
xlabel(20(5)50) legend(off) ///
saving($result/mismatch_by_age, replace)
graph export $result/mismatch_by_age.eps, replace

twoway lpoly mm age if age <= 55 & age >=18, /// 
ytitle("Mismatch Measure") xtitle("Age") ///
title("Mismatch Over the Life-Cycle") ///
xlabel(20(5)55) legend(off)  lwidth(thick)  ///
saving($result/mismatch_by_age, replace)
graph export $result/mismatch_by_age_noci.eps, replace
...
/* Switch probabiliy by mm_pos mm_neg*/
twoway (lpoly switch_occ mm_pos, lwidth(thick)) (lpoly switch_occ mm_neg, lwidth(thick)) if age<=50 & age>=18 , ///
title("Signed Mismatch and Switching") ///
xtitle("Signed Mismatch Components") ///
ytitle("Probability of Switching Occupations") ///
legend(label(1 "Positive Components") label(2 "Negative Components")) ///
saving($result/p_mm_sgn_switch_by_age, replace)
graph export $result/p_mm_sgn_switch_by_age.eps, replace



***DO THESE PROFILES BY REGRESSIONS

gen age2 = age*age
gen age3 = age2*age

gen ONET_age = ONET_m*age
gen ONET_age2 = ONET_m*age2
gen ONET_age3 = ONET_m*age3

matrix mmeff_age = J(33,2,0.0)
qui reg mm _I_age* _I_yob1-_I_yob4 _I_yob6-_I_yob9 univ lths asvab_m ///
ONET_m ONET_age ONET_age2 if age>=18 & age<=50
forvalues ai=1/33{
	matrix mmeff_age[`ai',1] = `ai'+17
	matrix mmeff_age[`ai',2] = _b["_I_age`ai'"] + _b["_cons"]
}
svmat mmeff_age
twoway line mmeff_age2 mmeff_age1, ///
ytitle("Mismatch Measure") ///
xtitle("Age") lwidth(thick) ///
title("Mismatch by age") ///
saving($result/mismatch_by_age_dummies, replace)
graph export $result/mismatch_by_age_dummies.eps, replace

matrix mmpoly_age = mmeff_age
qui reg mm age age2 age3  _I_yob1-_I_yob4 _I_yob6-_I_yob9 univ lths asvab_m ///
ONET_m ONET_age ONET_age2 if age>=18 & age<=50
forvalues ai=1/33{
	matrix mmpoly_age[`ai',1] = `ai'+17
	matrix mmpoly_age[`ai',2] = _b["age"]*(`ai'+17) + _b["age2"]*(`ai'+17)^2+_b["age3"]*(`ai'+17)^3 + _b["_cons"]
}
svmat mmpoly_age
twoway (line mmpoly_age2 mmpoly_age1,  lwidth(thick)) (scatter mmeff_age2 mmeff_age1), ///
ytitle("Mismatch Measure") ///
xtitle("Age") legend(off) ///
title("Mismatch by age") ///
saving($result/mismatch_by_age_poly, replace)
graph export $result/mismatch_by_age_poly.eps, replace

/* Mismatch super-imposed on switch pr */
twoway (lpoly switch_occ age if age >= 20 & age <= 50, yaxis(1) ylabel(0.05(0.05)0.3) ytitle("Probability of a Switch", axis(1)) lwidth(thick) ) ///
(line mmpoly_age2 mmpoly_age1 if mmpoly_age1>=20 & mmpoly_age1<=50, yaxis(2) ytitle("Mismatch", axis(2)) lwidth(thick)) ////
(scatter mmeff_age2 mmeff_age1  if mmeff_age1>=20 & mmeff_age1<=50, yaxis(2) ), ///
xtitle("Age") ///
title("Occupational Switches Over the Life-Cycle") ///
xlabel(20(5)50) legend(label(1 "Pr[Switch]") label(2 "Mismatch -fitted") label(3 "Mismatch") order(1 3 2))  /// legend(off) 
saving($result/p_mm_switch_by_age_poly, replace)
graph export $result/p_mm_switch_by_age_poly.eps, replace


drop age? ONET_age* mmpoly_age* mmeff_age*

** How big is the effect of mismatch over age:

* at the mean
estimate use $result/iv_cmm_mm_asvab_onet.ster
qui sum mm if age>=20 & age <=22, meanonly
local mm_y = r(mean)
qui sum tenure_occ , meanonly
local ten_y = r(mean)
local mm_effect_y = _b["mm"]*`mm_y' + _b["mm_ten_occ"]*`mm_y'*`ten_y'
qui sum mm if age>=48 & age <=52, meanonly
local mm_old = r(mean)
qui sum tenure_occ , meanonly
local ten_old = r(mean)
local mm_effect_old = _b["mm"]*`mm_old' + _b["mm_ten_occ"]*`mm_old'*`ten_old'

disp `mm_effect_old'-`mm_effect_y'


* at the top and bottom quintiles
xtset id year
by id: egen tmp_mm_y = mean(mm) if age>=18 & age<=22
by id: egen tmp_mm_o = mean(mm) if age>=48 & age<=52
by id: egen tmp2_mm_y = max(tmp_mm_y)
by id: egen tmp2_mm_o = max(tmp_mm_o)
gen totchng_mm = tmp2_mm_o - tmp2_mm_y
drop tmp*_mm_*
_pctile totchng_mm, p(20 80)
local chng_botquint = r(r1)
twoway lpoly mm age if totchng_mm<`chng_botquint' & age>=18 & age<=50, lwidth(thick) ///
ytitle("Mismatch Measure") ///
xtitle("Age") legend(off) ///
title("Mismatch by age - 20% largest fall") ///
saving($result/mismatch_by_age_botquint, replace)
graph export $result/mismatch_by_age_botquint.eps, replace


qui reg mm age age2 age3  _I_yob1-_I_yob4 _I_yob6-_I_yob9 univ lths asvab_m ///
ONET_m ONET_age ONET_age2 if age>=18 & age<=50 & totchng_mm<=`chng_botquint'
forvalues ai=1/33{
	matrix mmpoly_age[`ai',1] = `ai'+17
	matrix mmpoly_age[`ai',2] = _b["age"]*(`ai'+17) + _b["age2"]*(`ai'+17)^2+_b["age3"]*(`ai'+17)^3 + _b["_cons"]
}
...
estimate use $result/iv_cmm_mm_asvab_onet.ster
qui sum mm if age>=20 & age <=22 & totchng_mm<=`chng_botquint', meanonly
local mm_y = r(mean)
qui sum tenure_occ if totchng_mm<=`chng_botquint', meanonly
local ten_y = r(mean)
local mm_effect_y = _b["mm"]*`mm_y' + _b["mm_ten_occ"]*`mm_y'*`ten_y'
qui sum mm if age>=48 & age <=52 & totchng_mm<=`chng_botquint', meanonly
local mm_old = r(mean)
qui sum tenure_occ if totchng_mm<=`chng_botquint', meanonly
local ten_old = r(mean)
local mm_effect_old = _b["mm"]*`mm_old' + _b["mm_ten_occ"]*`mm_old'*`ten_old'
...
twoway lpoly switch_occ match, /// ci 
ytitle("Probability of Occupational Switch") ///
xtitle("Match Quality") lwidth(thick) ///
title("Probability of Occupational Switch") ///
xlabel(-.6(.2).6) ylabel(.15(.05).3) legend(off) ///
saving($result/u-shape_match, replace)
graph export $result/u-shape_match.eps, replace

twoway lpoly switch_occ mm,  /// ci 
ytitle("Probability of Occupational Switch") ///
xtitle("Mismatch") lwidth(thick) ///
title("Probability of Occupational Switch") ///
legend(off) ///
