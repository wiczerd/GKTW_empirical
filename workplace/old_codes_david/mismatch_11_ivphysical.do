...
if($hlth_idx_use ==1){
	...
	*reg hlth_composite bmi_init bdad_alive bdad_hlth bdad_death bmom_alive bmom_hlth bmom_death hlth_tv hlth_smoke hlth_alchl_days_max black hispanic i.occ_1d i.ind_1d
	reg hlth_composite bmi_init height bdad_alive bdad_death bmom_alive bmom_death $zlist
	predict hlth_resid, resid
	sum hlth_resid
	replace ability_h = (hlth_resid - r(min))/r(sd)
	drop *_tmp
}
...
outreg2 using $result/table_vmm_cmm.tex, bdec(3) tex(fragment) ctitle("", " ") drop(_I* $zlist) label se sortvar(match_* absmm_* mm mm_* cmatch_* cmm_* $xlist $zlist)
...
estimate use $result/iv_ind_match_means.ster
matrix eb=e(b)
matrix pred_match = J(16,1+3*`ndim',0.0)
local j = 0
...
if($twodim == 1){
	...
	saving($result/pred_effect_ind_match_means, replace)
}
...
graph export $result/pred_effect_ind_match_means.eps, replace
drop pred_match*
...
if($twodim == 1){
	...
	lpattern("l" "l" "l" "l" "l" "l") lcolor("red" "red" "red" "blue" "blue" "blue") ///
	legend(label(1 "90 pctl") label(2 "50 pctl") label(3 "10 pctl") label(4 "90 pctl") label(5 "50 pctl") label(6 "10 pctl") c(2) order(1 4 2 5 3 6) ) ///
	saving($result/pred_effect_ind_mm_means, replace)
}
...
graph export $result/pred_effect_ind_mm_means.eps, replace
drop pred_match*


graph combine $result/pred_effect_ind_match_means.gph $result/pred_effect_ind_mm_means.gph, ycommon
graph export $result/pred_effect_ind_match_mm_means.eps, replace
...
use $result/yearly_03_phys.dta, clear
foreach i of local nlist{
	_pctile cmatch_cur_`i', p(10 30 50 70 90)
	local p90_`i' = r(r5)
	local p70_`i' = r(r4)
	local p50_`i' = r(r3)
	local p30_`i' = r(r2)
...
foreach ii of local nlist {
	forvalues tt=0/15{
		matrix pred_match[`tt'+1,1] = `tt'
		matrix pred_match[`tt'+1,2+`j'*3] = _b["cmatch_cur_`ii'"]*`p90_`ii''
		matrix pred_match[`tt'+1,3+`j'*3] = _b["cmatch_cur_`ii'"]*`p50_`ii''
		matrix pred_match[`tt'+1,4+`j'*3] = _b["cmatch_cur_`ii'"]*`p10_`ii''
	}
	local j = `j'+1
}
svmat pred_match
...
if($twodim == 1){
	...
	lpattern("l" "l" "l" "l" "l" "l") lcolor("red" "red" "red" "blue" "blue" "blue") ///
	legend(label(1 "90 pctl") label(2 "50 pctl") label(3 "10 pctl") label(4 "90 pctl") label(5 "50 pctl") label(6 "10 pctl") c(2) order(1 4 2 5 3 6) ) ///
	saving($result/pred_effect_ind_cmatch_means, replace)
}
...
graph export $result/pred_effect_ind_cmatch_means.eps, replace
drop pred_match*
...
use $result/yearly_03_phys.dta, clear
foreach i of local nlist{
	_pctile cmm_cur_`i', p(10 30 50 70 90)
	local p90_`i' = r(r5)
	local p70_`i' = r(r4)
	local p50_`i' = r(r3)
	local p30_`i' = r(r2)
...
foreach ii of local nlist {
	forvalues tt=0/15{
		matrix pred_match[`tt'+1,1] = `tt'
		matrix pred_match[`tt'+1,2+`j'*3] = _b["cmm_cur_`ii'"]*`p90_`ii''
		matrix pred_match[`tt'+1,3+`j'*3] = _b["cmm_cur_`ii'"]*`p50_`ii''
		matrix pred_match[`tt'+1,4+`j'*3] = _b["cmm_cur_`ii'"]*`p10_`ii''
	}
	local j = `j'+1
}
svmat pred_match
...
if($twodim == 1){
	...
	lpattern("l" "l" "l" "l" "l" "l") lcolor("red" "red" "red" "blue" "blue" "blue") ///
	legend(label(1 "90 pctl") label(2 "50 pctl") label(3 "10 pctl") label(4 "90 pctl") label(5 "50 pctl") label(6 "10 pctl") c(2) order(1 4 2 5 3 6) ) ///
	saving($result/pred_effect_ind_cmm_means, replace)
}
...
graph export $result/pred_effect_ind_cmm_means.eps, replace
drop pred_match*


graph combine $result/pred_effect_ind_cmatch_means.gph $result/pred_effect_ind_cmm_means.gph, ycommon
graph export $result/pred_effect_ind_cmatch_cmm_means.eps, replace
