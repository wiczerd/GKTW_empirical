...
local llist "Verbal Math Social"
local nlist "aa bb cc"
local cllist "eltblue eltblue eltblue"
local tllist "(a) (b) (c)"
local ll =1
local cll = 1
local tll = 1
foreach i of local nlist {
	local l: word `ll' of `llist'
	local c: word `cll' of `cllist'
	local t: word `tll' of `tllist'
	graph twoway (scatter chng_skill_`i' lmm_`i'_pos if switch_occ==1 & lmm_`i'_pos > 0, msymbol(x) mlcolor("`c'") mlwidth(vvthin)) ///
                     (scatter chng_skill_`i' lmm_`i'_neg if switch_occ==1 & lmm_`i'_neg < 0, msymbol(x) mlcolor("`c'") mlwidth(vvthin)) ///
