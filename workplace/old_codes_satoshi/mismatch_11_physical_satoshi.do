...
quietly forvalues i = 1/3 {
	...
	gen rsum = sum(swt)
	sum rsum, meanonly
	gen skill_rnk_`i' = rsum/r(max)
	gsort -rsum
	replace skill_rnk_`i' = skill_rnk_`i'[_n-1] if skill_`i' == skill_`i'[_n-1]
	drop rsum
}
...
quietly forvalues i = 1/3 {
	...
	gen rsum = sum(swt)
	sum rsum, meanonly
	gen ability_rnk_`i' = rsum/r(max)
	gsort -rsum
	replace ability_rnk_`i' = ability_rnk_`i'[_n-1] if ability_`i' == ability_`i'[_n-1]
	drop rsum
}
...
gen ability_skill_ten_occ = ability_m*skill_m*tenure_occ
gen ability_skill_ten_occ_iv = ability_m*skill_m*ten_occ_iv
...
xtset id year
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj
xi: reg lwage match match_ten_occ ability_skill_m ability_m skill_m ability_skill_ten_occ $xlist $zlist i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
global xlist match_ten_occ tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj ability_skill_ten_occ
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv match_ten_occ_iv ability_skill_ten_occ_iv
xi: ivregress 2sls lwage match ability_skill_m ability_m skill_m ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)
