/*--------------------------------------------------------------------------------------
* name: mismatch_11_satoshi_xdimphysical.do
...
if($hlth_idx_use ==1){
	...
	global hlth_list bmi_init_p25 bmi_init_m25 bmi_init2 height yob bdad_alive bdad_death bmom_alive bmom_death $zlist
...
estimate store ivten_mm_means
estimate use ${result}/ivten_cmm_mm_means.ster
estimate store ivten_cmm_mm_means
estimate use ${result}/ivten_cmm_mm_means_pos_neg.ster
estimate store ivten_cmm_mm_means_pos_neg
...
esttab ols_mm_means ols_cmm_mm_means ols_cmm_mm_means_pos_neg ivten_mm_means ivten_cmm_mm_means ivten_cmm_mm_means_pos_neg iv_mm_means iv_cmm_mm_means iv_cmm_mm_means_pos_neg ///
                   using ${result}/table_${diminitls}.tex, b(3) se(3) ///
                   r2 noconstant nodepvars label nogap compress substitute(\hline\hline \hline \hline "\noalign{\vskip 1mm} \hline \noalign{\vskip 1mm}" "Standard" "Robust standard") ///
		   drop(_I* oj ten* exp* $zlist) ///
		   mtitles("1a" "1b" "1c" "2a" "2b" "2c" "3a" "3b" "3c") ///
