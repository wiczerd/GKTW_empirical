...
forvalues i=1/7{
	reg asvab_sec0`i' I_enterage1-I_enterage8
	gen constant = _b[_cons]
	qui predict asvab_res0`i', residual 
	replace asvab_res0`i' = asvab_res0`i' + constant
	drop constant
	}
...
gen sum_asvab = 0
forvalues i = 1/7 {
        quietly replace sum_asvab = sum_asvab + asvab_rnk`i'/7
}
/*
gen sum_asvab = 0
forvalues i = 1/7 {
...
forvalues i=1/7{
	sort ONET_ASVAB_`i'
	qui gen rsum = sum(ones)
	qui sum rsum, meanonly
	gen ONET_rnk_`i' = rsum/r(max)
	qui gsort -rsum
	qui replace ONET_rnk_`i' = ONET_rnk_`i'[_n-1] if ONET_rnk_`i'== ONET_rnk_`i'[_n-1]
	drop rsum
}
/*
gen ones = 1
...
gen sum_ONET = 0
forvalues i=1/7{
        qui replace sum_ONET = sum_ONET + ONET_rnk_`i'/7
}
/*
gen sum_ONET = 0
forvalues i=1/7{
...
gen sum_ONET_asvab = sum_ONET*sum_asvab

sort sum_ONET_asvab
quietly gen rsum = sum(one)
quietly sum rsum, meanonly
quietly gen sum_ONET_asvab_rnk = rsum/r(max)
quietly gsort -rsum
quietly replace sum_ONET_asvab_rnk = sum_ONET_asvab_rnk[_n-1] if sum_ONET_asvab == sum_ONET_asvab[_n-1]
drop rsum

sort sum_ONET
quietly gen rsum = sum(one)
quietly sum rsum, meanonly
quietly gen sum_ONET_rnk = rsum/r(max)
quietly gsort -rsum
quietly replace sum_ONET_rnk = sum_ONET_rnk[_n-1] if sum_ONET == sum_ONET[_n-1]
drop one rsum
...
gen asvab_exp = sum_asvab_rnk*exp
gen asvab_exp_iv = sum_asvab_rnk*exp_iv

gen ONET_asvab_ten_occ = sum_ONET_asvab_rnk*tenure_occ
gen ONET_asvab_ten_occ_iv = sum_ONET_asvab_rnk*ten_occ_iv
...
label var asvab_ten_occ2 "ASVAB $\times$ Occ Tenure$^2$"
label var ONET_ten_occ2 "ONET $\times$ Occ Tenure"

label var asvab_exp "ASVAB $\times$ Experience"

label var sum_ONET_asvab_rnk "ASVAB $\times$ ONET"
label var ONET_asvab_ten_occ "ASVAB $\times$ ONET $\times$ Occ Tenure"
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2 $zlist sum_asvab_rnk sum_ONET_rnk sum_ONET_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2 $zlist sum_asvab_rnk asvab_ten_occ sum_ONET_rnk ONET_ten_occ sum_ONET_asvab_rnk ONET_asvab_ten_occ i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2 $zlist sum_asvab_rnk asvab_ten_occ asvab_exp sum_ONET_rnk ONET_ten_occ sum_ONET_asvab_rnk ONET_asvab_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_asvab_ONET_occten_exp.ster, replace
outreg using $result/table_corrols.tex, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2 $zlist sum_ONET_rnk ONET_ten_occ ONET_ten_occ2 sum_asvab_rnk asvab_ten_occ asvab_ten_occ2 i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_ONET_ASVAB_occten2.ster, replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist match match_ten_occ match_ten_occ2 cmatch cmatch_exp $zlist sum_asvab_rnk asvab_ten_occ sum_ONET_rnk ONET_ten_occ i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
qui xi: reg lwage $xlist mm mm_ten_occ mm_ten_occ2 $zlist i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist mm $zlist sum_asvab_rnk sum_ONET_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_asvab_ONET_mm.ster, replace
outreg , tex merge bdec(3) ctitle("", "(2)") drop(_I* $zlist ) fragment varlabels se replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist mm mm_ten_occ $zlist sum_asvab_rnk asvab_ten_occ sum_ONET_rnk ONET_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_asvab_ONET_occten_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(3)") fragment drop(_I* $zlist ) varlabels se replace
...
keep if $sample_select
xtset id year
xi: reg lwage $xlist mm mm_ten_occ match_ten_occ2 $zlist sum_ONET_rnk ONET_ten_occ ONET_ten_occ2 sum_asvab_rnk asvab_ten_occ asvab_ten_occ2 i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_ONET_ASVAB_occten2_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace


/*------------------------------------------------------------------------------------*/

/* ols with mmmatch + cumulative match + sum of ONET score + sum of ASVAB score */

use $result/yearly_03_corr.dta, clear
keep if $sample_select
xtset id year
xi: reg lwage $xlist mm mm_ten_occ cmm cmm_exp $zlist sum_asvab_rnk asvab_ten_occ sum_ONET_rnk ONET_ten_occ i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corrols_cmatch.ster, replace
outreg using $result/table_corrols.tex, tex merge bdec(3) ctitle("", "(5)") fragment drop(_I* $zlist ) varlabels se replace
*/
...
qui xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist i.ind_1d i.occ_1d, vce(robust)
...
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv match_ten_occ_iv match_ten_occ2_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist sum_asvab_rnk sum_ONET_rnk sum_ONET_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
...
keep if $sample_select
xtset id year
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2 asvab_ten_occ ONET_ten_occ ONET_asvab_ten_occ
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  match_ten_occ_iv match_ten_occ2_iv asvab_ten_occ_iv ONET_ten_occ_iv ONET_asvab_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist sum_asvab_rnk sum_ONET_rnk sum_ONET_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corriv_asvab_ONET_occten.ster, replace
outreg using $result/table_corriv.tex, tex merge bdec(3) ctitle("", "(3)") drop(_I* $zlist ) fragment varlabels se replace
...
keep if $sample_select
xtset id year
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2 asvab_ten_occ asvab_exp ONET_ten_occ ONET_asvab_ten_occ
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  match_ten_occ_iv match_ten_occ2_iv asvab_ten_occ_iv asvab_exp_iv ONET_ten_occ_iv ONET_asvab_ten_occ_iv
xi: ivregress 2sls lwage ($xlist = $ivlist) match $zlist sum_asvab_rnk sum_ONET_rnk sum_ONET_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corriv_asvab_ONET_occten.ster, replace
outreg using $result/table_corriv.tex, tex merge bdec(3) ctitle("", "(4)") drop(_I* $zlist ) fragment varlabels se replace
...
keep if $sample_select
xtset id year
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2 asvab_ten_occ ONET_ten_occ ONET_ten_occ2 asvab_ten_occ2 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv  match_ten_occ_iv match_ten_occ2_iv asvab_ten_occ_iv ONET_ten_occ_iv ONET_ten_occ2_iv asvab_ten_occ2_iv
xi: ivregress 2sls lwage  ($xlist = $ivlist) match $zlist sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d, vce(robust)
estimate save $result/corriv_ONET_ASVAB_occten2.ster, replace
outreg, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace
*/
...
keep if $sample_select
xtset id year
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj match_ten_occ match_ten_occ2 asvab_ten_occ ONET_ten_occ 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv match_ten_occ_iv match_ten_occ2_iv asvab_ten_occ_iv ONET_ten_occ_iv 
qui xi: ivregress 2sls lwage ($xlist = $ivlist) match cmatch cmatch_exp $zlist  sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/corriv_cmatch.ster, replace
outreg using $result/table_corriv.tex, tex merge bdec(3) ctitle("", "(5)") fragment drop(_I* $zlist ) varlabels se replace
*/
...
qui xi: ivregress 2sls lwage ($xlist = $ivlist) mm $zlist i.ind_1d i.occ_1d, vce(robust)
...
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv 
xi: ivregress 2sls lwage ($xlist = $ivlist) mm $zlist sum_asvab_rnk sum_ONET_rnk i.ind_1d i.occ_1d, vce(robust)
...
estimate save $result/iv_ONET_ASVAB_iccten2_mm.ster, replace
outreg, tex merge bdec(3) ctitle("", "(4)") fragment drop(_I* $zlist ) varlabels se replace
...
global xlist tenure_emp ten_emp2 tenure_occ ten_occ2 ten_occ3 exp exp2 exp3 oj mm_ten_occ asvab_ten_occ ONET_ten_occ 
global ivlist ten_emp_iv ten_emp2_iv ten_occ_iv ten_occ2_iv ten_occ3_iv exp_iv exp2_iv exp3_iv oj_iv mm_ten_occ_iv asvab_ten_occ_iv ONET_ten_occ_iv 
qui xi: ivregress 2sls lwage ($xlist = $ivlist) match cmm cmm_exp $zlist  sum_ONET_rnk sum_asvab_rnk i.ind_1d i.occ_1d , vce(robust)
estimate save $result/iv_cmm.ster, replace
outreg using $result/table_iv_mm.tex, tex merge bdec(3) ctitle("", "(5)") fragment drop(_I* $zlist ) varlabels se replace
