...
predict loguhat2, residuals
replace loguhat2 = loguhat2^2
xtreg loguhat2 l.loguhat2 , fe /* mm cmm  $zlist ability_mean skill_mean i.ind_1d i.occ_1d ${xlist} */
global rhohat = _b["L.loguhat2"]
replace loguhat2 = log(loguhat2)
xtreg loguhat2 l.loguhat2 , fe /* mm cmm  $zlist ability_mean skill_mean i.ind_1d i.occ_1d ${xlist} */
predict inv_omega, xbu
predict varresid, e
by id: egen sd_er_resid = sd(varresid)
replace inv_omega = exp(inv_omega+sd_er_resid^2/2)
foreach zv of varlist mm cmm $zlist ability_mean skill_mean $xlist $ivlist lwage{
	gen `zv'_R = `zv'
	replace `zv' = `zv'_R - ${rhohat}*l.`zv'_R
	_crcslbl `zv'_R `zv'
}

*xi: ivregress 2sls lwage mm cmm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d [pw = 1/inv_omega]
*xi: ivregress 2sls lwage_yR (*_xR = *_iR) *_R i.ind_1d i.occ_1d [aw = 1/inv_omega]
xi: ivregress 2sls lwage mm cmm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d [aw = 1/inv_omega]
...
drop inv_omega loguhat2 sd_er_resid varresid
drop *_R 
...
global xlist  mm_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
xtset id year
xi: xtreg lwage mm cmm $xlist  skill_mean i.ind_1d i.occ_1d, fe vce(${vcetxt})
estimate save ${result}/ols_cmm_mm_fe.ster, replace

global xlist  mm_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
global ivlist mm_ten_occ_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv $ivlist_0
if("${varcor}" == "cid" ){
	xtivreg2 lwage mm cmm ($xlist = $ivlist)  skill_mean _Iind_1d_* _Iocc_1d_*, cluster(id) fe
} 
else{
...
replace inv_omega = exp(inv_omega+sd_er_resid^2/2)
*xi: ivregress 2sls lwage mm cmm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d [pw = 1/inv_omega]
foreach zv of varlist mm cmm  skill_mean  $xlist $ivlist lwage{
	gen `zv'_R = `zv'
	replace `zv' = `zv'_R  - ${rhohat}*l.`zv'_R 
	_crcslbl `zv'_R `zv'
}

*xtivreg2 lwage mm cmm  skill_mean _Iind_1d_* _Iocc_1d_* ($xlist = $ivlist) [pw = 1/inv_omega], fe
*xtivreg2 lwage_yR *_R _Iind_1d_* _Iocc_1d_* (*_xR = *_iR) [aw = 1/inv_omega], fe
xtivreg2 lwage mm cmm  skill_mean _Iind_1d_* _Iocc_1d_* ($xlist = $ivlist) [aw = 1/inv_omega], fe
...
*if("$varmeth" == "boot" & "$varcor" == "cid"){
*	xtset,clear
*	bootstrap, cluster(id)  seed(9487): ivregress 2sls lwage mm_2nm cmm_2nm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d
*	xtset id year
* } 
else{
...
if("$varmeth" == "boot" & "$varcor" == "cid"){
	xtset, clear
	bootstrap, cluster(id) seed(9487): ivregress 2sls lwage mm_pos mm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d 
...
if("$varmeth" == "boot" & "$varcor" == "cid"){
	xtset, clear
	bootstrap, cluster(id)  seed(9487): ivregress 2sls lwage mm_pos mm_neg cmm_pos cmm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d
...
replace inv_omega = exp(inv_omega+sd_er_resid^2/2)
*xi: ivregress 2sls lwage mm cmm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d [pw = 1/inv_omega]
foreach zv of varlist mm_pos mm_neg cmm_pos cmm_neg $zlist $xlist $ivlist lwage{
	gen `zv'_R = `zv'
	replace `zv' = `zv'_R  - ${rhohat}*l.`zv'_R 
	_crcslbl `zv'_R `zv'
}

*xi: ivregress 2sls lwage mm_pos mm_neg cmm_pos cmm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d [pw = 1/inv_omega] 
*xi: ivregress 2sls lwage_yR *_R (*_xR = *_iR) i.ind_1d i.occ_1d [aw = 1/inv_omega] 
xi: ivregress 2sls lwage mm_pos mm_neg cmm_pos cmm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d [aw = 1/inv_omega] 
...
global xlist  mm_pos_ten_occ mm_neg_ten_occ $xlist_0
xtset id year
xtreg lwage mm_pos mm_neg cmm_pos cmm_neg $xlist  _Iind_1d* _Iocc_1d*, vce(${vcetxt}) fe
...
*xtivreg2 lwage mm_pos mm_neg cmm_pos cmm_neg _Iind_1d* _Iocc_1d* ($xlist = $ivlist) [pw = 1/inv_omega] , fe
*xtivreg2 lwage_yR *_R _Iind_1d* _Iocc_1d* (*_xR = *_iR) [pw = 1/inv_omega] , fe
xtivreg2 lwage mm_pos mm_neg cmm_pos cmm_neg _Iind_1d* _Iocc_1d* ($xlist = $ivlist) [pw = 1/inv_omega] , fe
...
if("$varmeth" == "boot" & "$varcor" == "cid"){
	xtset,clear
	bootstrap, cluster(id)  seed(9487): ivregress 2sls lwage absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d 
...
*xi: ivregress 2sls lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d [pw = 1/inv_omega], 
*xi: ivregress 2sls lwage_yR *_R (*_xR = *_iR) i.ind_1d i.occ_1d [aw = 1/inv_omega]
xi: ivregress 2sls lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d [aw = 1/inv_omega], 
...
global xlist  absmm_??_ten_occ ability_??_ten_occ skill_??_ten_occ $xlist_0 
xtset id year
xtreg lwage cmm_aa cmm_bb cmm_cc absmm_?? $xlist   skill_?? _Iind_1d* _Iocc_1d*, vce(${vcetxt}) fe
...
if("$varmeth" == "boot" & "${varcor}"=="cid"){
	xtset, clear
	bootstrap, cluster(id)  seed(9487): xtivreg2 lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist) skill_?? _Iind_1d* _Iocc_1d*, i(id) fe
...
*xtivreg2 lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist)  skill_?? _Iind_1d* _Iocc_1d* [pw = 1/inv_omega], fe
*xtivreg2 lwage_yR *_R (*_xR = *_iR)  _Iind_1d* _Iocc_1d* [aw = 1/inv_omega], fe
xtivreg2 lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist)  skill_?? _Iind_1d* _Iocc_1d* [aw = 1/inv_omega], fe
...
esttab iv_mm iv_mm_ten iv_cmm_mm iv_cmm_mm_fe ols_cmm_mm ols_cmm_mm_fe ///
		   ...
		   order(mm mm_ten_occ cmm ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ ten* exp* oj $zlist _cons) ///
                   star(\sym{*} 0.10 \sym{**} 0.05 \sym{***} 0.01) replace
*FGLS
esttab iv_cmm_mm_rbst iv_cmm_mm_fe_rbst iv_cmm_mm iv_cmm_mm_fe iv_cmm_mm_boot iv_cmm_mm_fe_boot iv_cmm_mm_fgls iv_cmm_mm_fe_fgls ///
		   ...
		   mtitles("IV-RBST" "IV-FE-RBST" "IV-CLU" "IV-FE-CLU"  "IV-BOOT" "IV-FE-BOOT" "IV-FGLS" "IV-FE-FGLS") ///
		   order(mm mm_ten_occ cmm ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ ten* exp* oj $zlist _cons) ///
                   star(\sym{*} 0.10 \sym{**} 0.05 \sym{***} 0.01) replace
...
*FGLS
esttab iv_cmm_mm_pos_neg_rbst iv_cmm_mm_pos_neg_fe_rbst iv_cmm_mm_pos_neg iv_cmm_mm_pos_neg_fe iv_cmm_mm_pos_neg_boot iv_cmm_mm_pos_neg_fe_boot iv_cmm_mm_pos_neg_fgls iv_cmm_mm_pos_neg_fe_fgls ///
...
*FGLS
esttab iv_ind_cmm_mm_rbst iv_ind_cmm_mm_fe_rbst iv_ind_cmm_mm iv_ind_cmm_mm_fe iv_ind_cmm_mm_boot iv_ind_cmm_mm_fe_boot iv_ind_cmm_mm_fgls iv_ind_cmm_mm_fe_fgls ///
