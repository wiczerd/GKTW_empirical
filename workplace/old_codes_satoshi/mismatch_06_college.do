...
forvalues cg=0/1{
	...
	if(`cg'== 1){
		global diminitls "vms_cg"
		/* drop non-college */
		keep if grade > 12
	}
	else{
		global diminitls "vms_nc"
		/* drop non-college */
		keep if grade <= 12
	}
	...
	estimate save ${result}/bench_ols_${diminitls}.ster, replace
	...
	estimate save ${result}/bench_iv_${diminitls}.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_mm.ster, replace
	...
	estimate save ${result}/iv_${diminitls}_mm.ster, replace
	...
	xi: reg lwage mm $xlist $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/ols_${diminitls}_mm_ten.ster, replace

	global xlist  mm_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
	global ivlist mm_ten_occ_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage mm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/iv_${diminitls}_mm_ten.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_mm_pos_neg.ster, replace

	global xlist  $xlist_0
	global ivlist $ivlist_0
	xi: ivregress 2sls lwage mm_pos mm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/iv_${diminitls}_mm_pos_neg.ster, replace
	...
	xi: reg lwage mm_pos mm_neg $xlist $zlist i.ind_1d i.occ_1d, vce(robust)
	estimate save ${result}/ols_${diminitls}_mm_ten_pos_neg.ster, replace

	global xlist  mm_pos_ten_occ mm_neg_ten_occ $xlist_0
	global ivlist mm_pos_ten_occ_iv mm_neg_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage mm_pos mm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/iv_${diminitls}_mm_ten_pos_neg.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_cmm_mm.ster, replace

	global xlist  mm_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
	global ivlist mm_ten_occ_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage mm cmm ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(robust)
	estimate save ${result}/iv_${diminitls}_cmm_mm.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_cmm_mm_pos_neg.ster, replace

	global xlist  mm_pos_ten_occ mm_neg_ten_occ $xlist_0
	global ivlist mm_pos_ten_occ_iv mm_neg_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage mm_pos mm_neg cmm_pos cmm_neg ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d, vce(robust)
	estimate save ${result}/iv_${diminitls}_cmm_mm_pos_neg.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_ind_mm.ster, replace
	...
	estimate save ${result}/iv_${diminitls}_ind_mm.ster, replace
	...
	xi: reg lwage absmm_?? $xlist $zlist ability_?? skill_?? i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/ols_${diminitls}_ind_mm_ten.ster, replace

	global xlist  absmm_??_ten_occ ability_??_ten_occ skill_??_ten_occ $xlist_0
	global ivlist absmm_??_ten_occ_iv ability_??_ten_occ_iv skill_??_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d , vce(robust)
	estimate save ${result}/iv_${diminitls}_ind_mm_ten.ster, replace
	...
	estimate save ${result}/ols_${diminitls}_ind_cmm_mm.ster, replace

	global xlist  absmm_??_ten_occ ability_??_ten_occ skill_??_ten_occ $xlist_0 
	global ivlist absmm_??_ten_occ_iv ability_??_ten_occ_iv skill_??_ten_occ_iv $ivlist_0
	xi: ivregress 2sls lwage cmm_aa cmm_bb cmm_cc absmm_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d, vce(robust)
	estimate save ${result}/iv_${diminitls}_ind_cmm_mm.ster, replace
	...
	estimate clear
	estimate use ${result}/ols_${diminitls}_mm.ster
	estimate store ols_${diminitls}_mm
	estimate use ${result}/ols_${diminitls}_mm_ten.ster
	estimate store ols_${diminitls}_mm_ten
	estimate use ${result}/ols_${diminitls}_cmm_mm.ster
	estimate store ols_${diminitls}_cmm_mm

	estimate use ${result}/iv_${diminitls}_mm.ster
	estimate store iv_${diminitls}_mm
	estimate use ${result}/iv_${diminitls}_mm_ten.ster
	estimate store iv_${diminitls}_mm_ten
	estimate use ${result}/iv_${diminitls}_cmm_mm.ster
	estimate store iv_${diminitls}_cmm_mm

	/* tex */		   
	esttab iv_${diminitls}_mm iv_${diminitls}_mm_ten iv_${diminitls}_cmm_mm ols_${diminitls}_mm ols_${diminitls}_mm_ten ols_${diminitls}_cmm_mm ///
	...
	estimate clear
	estimate use ${result}/ols_${diminitls}_mm_pos_neg.ster
	estimate store ols_${diminitls}_mm_pos_neg
	estimate use ${result}/iv_${diminitls}_mm_pos_neg.ster
	estimate store iv_${diminitls}_mm_pos_neg

	estimate use ${result}/ols_${diminitls}_mm_ten_pos_neg.ster
	estimate store ols_${diminitls}_mm_ten_pos_neg
	estimate use ${result}/iv_${diminitls}_mm_ten_pos_neg.ster
	estimate store iv_${diminitls}_mm_ten_pos_neg

	estimate use ${result}/ols_${diminitls}_cmm_mm_pos_neg.ster
	estimate store ols_${diminitls}_cmm_mm_pos_neg
	estimate use ${result}/iv_${diminitls}_cmm_mm_pos_neg.ster
	estimate store iv_${diminitls}_cmm_mm_pos_neg

	/* tex */
	esttab iv_${diminitls}_mm_pos_neg iv_${diminitls}_mm_ten_pos_neg iv_${diminitls}_cmm_mm_pos_neg ols_${diminitls}_mm_pos_neg ols_${diminitls}_mm_ten_pos_neg ols_${diminitls}_cmm_mm_pos_neg ///
	...
	estimate clear
	estimate use ${result}/ols_${diminitls}_ind_mm.ster
	estimate store ols_${diminitls}_ind_mm
	estimate use ${result}/ols_${diminitls}_ind_mm_ten.ster
	estimate store ols_${diminitls}_ind_mm_ten
	estimate use ${result}/ols_${diminitls}_ind_cmm_mm.ster
	estimate store ols_${diminitls}_ind_cmm_mm

	estimate use ${result}/iv_${diminitls}_ind_mm.ster
	estimate store iv_${diminitls}_ind_mm
	estimate use ${result}/iv_${diminitls}_ind_mm_ten.ster
	estimate store iv_${diminitls}_ind_mm_ten
	estimate use ${result}/iv_${diminitls}_ind_cmm_mm.ster
	estimate store iv_${diminitls}_ind_cmm_mm

	/* tex */
	esttab iv_${diminitls}_ind_mm iv_${diminitls}_ind_mm_ten iv_${diminitls}_ind_cmm_mm ols_${diminitls}_ind_mm ols_${diminitls}_ind_mm_ten ols_${diminitls}_ind_cmm_mm ///
...
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/* Tables and figures that compare college- and non-college */
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
...
estimate use ${result}/iv_vms_cg_mm.ster
estimate store iv_vms_cg_mm
estimate use ${result}/iv_vms_cg_mm_ten.ster
estimate store iv_vms_cg_mm_ten
estimate use ${result}/iv_vms_cg_cmm_mm.ster
estimate store iv_vms_cg_cmm_mm

estimate use ${result}/iv_vms_nc_mm.ster
estimate store iv_vms_nc_mm
estimate use ${result}/iv_vms_nc_mm_ten.ster
estimate store iv_vms_nc_mm_ten
estimate use ${result}/iv_vms_nc_cmm_mm.ster
estimate store iv_vms_nc_cmm_mm

/* tex */		   
esttab iv_vms_cg_mm iv_vms_cg_mm_ten iv_vms_cg_cmm_mm iv_vms_nc_mm iv_vms_nc_mm_ten iv_vms_nc_cmm_mm   ///
	           using ${result}/table_vms_cg_nc.tex, b(4) ///
	           r2 nodepvars gaps not label nonotes substitute(\hline\hline \hline \hline "\hline  " "Standard" "Robust standard" ///
	           "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("Col" "Col" "Col" "HS" "HS" "HS") ///
		   order(mm mm_ten_occ cmm ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ) ///
	           star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
		   
esttab iv_vms_cg_mm iv_vms_cg_mm_ten iv_vms_cg_cmm_mm iv_vms_nc_mm iv_vms_nc_mm_ten iv_vms_nc_cmm_mm   ///
	           using ${result}/table_apx_vms_cg_nc.tex, b(4) se(4) ///
	           r2 nodepvars nogaps label longtable substitute(\hline\hline \hline \hline "\hline  " ///
		   "Standard errors in parentheses" "All regressions include occupation and industry dummies." ///
		   "\sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)" "Robust standard errors in parentheses. \sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)." ///
	           "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   title("College v.s. Non-College: Wage Regressions with Mismatch (Full Results)") ///
		   drop(_I*) ///
		   mtitles("Col" "Col" "Col" "HS" "HS" "HS") ///
		   order(mm mm_ten_occ cmm ability_mean ability_mean_ten_occ skill_mean skill_mean_ten_occ ten* exp* oj $zlist _cons) ///
	           star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
...
estimate use ${result}/iv_vms_cg_ind_mm.ster
estimate store iv_vms_cg_ind_mm
estimate use ${result}/iv_vms_cg_ind_mm_ten.ster
estimate store iv_vms_cg_ind_mm_ten
estimate use ${result}/iv_vms_cg_ind_cmm_mm.ster
estimate store iv_vms_cg_ind_cmm_mm

estimate use ${result}/iv_vms_nc_ind_mm.ster
estimate store iv_vms_nc_ind_mm
estimate use ${result}/iv_vms_nc_ind_mm_ten.ster
estimate store iv_vms_nc_ind_mm_ten
estimate use ${result}/iv_vms_nc_ind_cmm_mm.ster
estimate store iv_vms_nc_ind_cmm_mm


/* tex */
esttab iv_vms_cg_ind_mm iv_vms_cg_ind_mm_ten iv_vms_cg_ind_cmm_mm iv_vms_nc_ind_mm iv_vms_nc_ind_mm_ten iv_vms_nc_ind_cmm_mm ///
		   using ${result}/table_vms_ind_cg_nc.tex, b(4) ///
		   r2 nodepvars gaps label not nonotes substitute(\hline\hline \hline \hline "\hline  " "Standard" "Robust standard" ///
		   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("Col" "Col" "Col" "HS" "HS" "HS") ///
...
esttab iv_vms_cg_ind_mm iv_vms_cg_ind_mm_ten iv_vms_cg_ind_cmm_mm iv_vms_nc_ind_mm iv_vms_nc_ind_mm_ten iv_vms_nc_ind_cmm_mm ///
		   using ${result}/table_apx_vms_ind_cg_nc.tex, b(4) se(4) ///
		   r2 nodepvars nogaps label longtable substitute(\hline\hline \hline \hline "\hline  " ///
		   "Standard errors in parentheses" "All regressions include occupation and industry dummies." ///
		   "\sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)" "Robust standard errors in parentheses. \sym{\sym{\dagger}} \(p<0.10\), \sym{\sym{*}} \(p<0.05\), \sym{\sym{**}} \(p<0.01\)." ///
		   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$") ///
		   drop(_I*) ///
		   mtitles("Col" "Col" "Col" "HS" "HS" "HS") ///
		   title("College v.s. Non-College: Wage Regressions with Mismatch by Component (Full Results)") ///
		   order(absmm_aa absmm_bb absmm_cc absmm_aa_t* absmm_bb_t* absmm_cc_t* cmm_aa cmm_bb cmm_cc ability_?? ability_??_* skill_?? skill_??_* ten* exp* oj $zlist _cons) ///
		   star(\sym{\dagger} 0.10 \sym{*} 0.05 \sym{**} 0.01) replace
