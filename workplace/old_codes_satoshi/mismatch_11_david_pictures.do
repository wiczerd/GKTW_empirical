...
/* creating a table */
estimate use $result/bench_iv.ster
outreg2 using $result/table_vmm_mm.tex, bdec(3) tex(fragment) title("Misatch Regression") ctitle("", "Benchmark IV") drop(_I* $zlist) label se replace sortvar(mm* $xlist $zlist)
...
/* Does it work with individual effects? (Test of AKM) */
global xlist  $xlist_0 cmatch_exp ability_mean_ten_occ skill_mean_ten_occ  
global ivlist $ivlist_0 cmatch_exp_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv 
xi: xtivreg lwage cmatch_cur ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, fe
...
global xlist  $xlist_0 cmm_exp ability_mean_ten_occ skill_mean_ten_occ  
global ivlist $ivlist_0 cmm_exp_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv 
xi: xtivreg lwage cmm_cur ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, fe


/* creating a table */
estimate use $result/bench_iv.ster
outreg2 using $result/table_vmm_cmm.tex, bdec(3) tex(fragment) title("Cumulative Misatch Regression") ctitle("", "Benchmark IV") drop(_I* $zlist) label se replace sortvar(match_* absmm_* cmatch_* cmm_* $xlist $zlist)
...
/* creating a table */
estimate use $result/bench_iv.ster
outreg2 using $result/table_vmm_ind_cmm.tex, bdec(3) tex(fragment) title("Cumulative Misatch Regression") ctitle("", "Benchmark IV") drop(_I* $zlist) label se replace sortvar(match_* absmm_* cmatch_* cmm_* $xlist $zlist)
...
global xlist  $xlist_0 ability_??_ten_occ skill_??_ten_occ  
global ivlist $ivlist_0 ability_??_ten_occ_iv skill_??_ten_occ_iv 
xi: xtivreg lwage cmm_cur_?? ($xlist = $ivlist) $zlist ability_?? skill_?? i.ind_1d i.occ_1d 
