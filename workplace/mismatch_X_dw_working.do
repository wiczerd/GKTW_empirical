...
/*-!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-*/		   
/* for referees */
* unemployment rate and mismatch (residual of age/demographics)
...
graphregion(color(white)) xlabel(, grid gstyle(dot)) ylabel(, grid gstyle(dot)) ///
legend(lab(1 "Residual mismatch") lab(2 "Moving-average of mismatch") lab(3 "Unemployment") ///
 ring(0) position(12) rows(3) ) ///
...
label var unrate_exp1 "Unemp Rt, experience 1"
drop urt0

gen urt0 = unrate if exp>=2 & exp<=5
by id: egen unrate_exp5 = mean(urt0)
label var unrate_exp5 "Unemp Rt, experience 2-5"
drop urt0

gen urt0 = unrate if exp>=6 & exp<=10
by id: egen unrate_exp10 = mean(urt0)
label var unrate_exp10 "Unemp Rt, experience 6-10"
drop urt0

reg mm unrate_exp1 $zlist i.age 
estimate store mm_urtexp1
reg mm unrate_exp1 unrate_exp5  $zlist i.age 
estimate store mm_urtexp5
...
/*-!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-*/		   
/* for referees */
* effect of experience on mismatch
*regression results for this:
reg mm exp $zlist_0 i.occ_1d i.ind_1d
estimate save ${result}/mm_exp.ster, replace

xtreg mm exp $zlist_0 i.occ_1d i.ind_1d
estimate save ${result}/mm_exp_re.ster, replace
qui{
	qreg mm exp $zlist_0 i.occ_1d i.ind_1d, q(.25)
	estimate save ${result}/mm_exp_q25.ster, replace

	qreg mm exp $zlist_0 i.occ_1d i.ind_1d, q(.5)
	estimate save ${result}/mm_exp_q50.ster, replace

	qreg mm exp $zlist_0 i.occ_1d i.ind_1d, q(.75)
	estimate save ${result}/mm_exp_q75.ster, replace
}

estimate use ${result}/mm_exp.ster
estimate store mm_exp
estimate use ${result}/mm_exp_re.ster
estimate store mm_exp_re
estimate use ${result}/mm_exp_q25.ster
estimate store mm_exp_q25
estimate use ${result}/mm_exp_q50.ster
estimate store mm_exp_q50
estimate use ${result}/mm_exp_q75.ster
estimate store mm_exp_q75



/* tex */		   
esttab mm_exp mm_exp_re mm_exp_q25 mm_exp_q50 mm_exp_q75 ///
                   using ${result}/tab_mm_exp_${diminitls}.tex, b(4) ///
                   nodepvars gaps not label nonotes substitute(\hline\hline \hline \hline "\hline  " "Standard" "${labtxt} standard" ///
                   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$" "\sym{\sym{***}}" "$^{***}$") ///
		   drop( *occ_* *ind_* _cons) ///
		   mtitles("OLS" "RE" "QR-0.25" "QR-0.50" "QR-0.75") ///
		   order(exp $zlist_0 ability_mean skill_mean) ///
                   star(\sym{*} 0.10 \sym{**} 0.05 \sym{***} 0.01) replace
...
/*-!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-*/		   
/* for referees */
/* relationship of mismatch to wage fixed effects for referee */
xtset id_occ year
xtivreg lwage _Iind_1d_* _Iocc_1d_* ($xlist = $ivlist), fe
predict fe_benchmark, u

twoway (lpolyci  fe_benchmark mm , ) if cmm<. & mm<5, ///
ytitle("Log Wage Fixed-Effect", size(large) color(dknavy)) ///
xtitle("Mismatch", size(large) color(dknavy)) ///
legend(off) ///
graphregion(color(white)) xlabel(,grid gstyle(default)) ylabel(, grid gstyle(default))
graph export ${result}/fe_mm.eps, replace
...
xi: reg lwage mm mm_lt35 $xlist  $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/ols_mm_young.ster, replace

xi: ivregress 2sls lwage mm mm_lt35 ($xlist  = $ivlist ) $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
...
xi: reg lwage mm mm_lt35 cmm cmm_lt35 $xlist  $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
estimate save ${result}/ols_cmm_mm_young.ster, replace

xi: ivregress 2sls lwage mm mm_lt35 cmm cmm_lt35 ($xlist  = $ivlist ) $zlist ability_mean skill_mean i.ind_1d i.occ_1d , vce(robust)
...
/* for referees */
/*------------------------------------------------------------------------------------*/
/* cumulative with all-dimensional mismatch */

global xlist  ability_mean_ten_occ skill_mean_ten_occ $xlist_0
xi: reg lwage mm_dim $xlist $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(${vcetxt})
estimate save ${result}/ols_mm_dim.ster, replace

global xlist  mm_dim_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
xi: reg lwage mm_dim $xlist $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(${vcetxt})
estimate save ${result}/ols_mm_ten_dim.ster, replace
...
global xlist  ability_mean_ten_occ skill_mean_ten_occ $xlist_0
global ivlist ability_mean_ten_occ_iv skill_mean_ten_occ_iv $ivlist_0
xi: ivregress 2sls lwage mm_dim ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(${vcetxt})
estimate save ${result}/iv_mm_dim.ster, replace

global xlist  mm_dim_ten_occ ability_mean_ten_occ skill_mean_ten_occ $xlist_0
global ivlist mm_dim_ten_occ_iv ability_mean_ten_occ_iv skill_mean_ten_occ_iv $ivlist_0
xi: ivregress 2sls lwage mm_dim ($xlist = $ivlist) $zlist ability_mean skill_mean i.ind_1d i.occ_1d, vce(${vcetxt})
estimate save ${result}/iv_mm_ten_dim.ster, replace
...
/* for referees */
/*------------------------------------------------------------------------------------*/		   
/* effect on youth robustness */
estimate clear
estimate use ${result}/iv_mm_young.ster
estimate store iv_mm_young
estimate use ${result}/iv_mm_ten_young.ster
...
estimate use ${result}/ols_mm_ten_young.ster
...
/* tex */		   
esttab iv_mm_young iv_mm_ten_young iv_cmm_mm_young ols_mm_young ols_mm_ten_young ols_cmm_mm_young ///
                   using ${result}/table_${diminitls}_young.tex, b(4) ///
                   r2 nodepvars gaps not label nonotes substitute(\hline\hline \hline \hline "\hline  " "Standard" "${labtxt} standard" ///
                   "\sym{\sym{\dagger}}" "$^{\dagger}$" "\sym{\sym{*}}" "$^{*}$" "\sym{\sym{**}}" "$^{**}$" "\sym{\sym{***}}" "$^{***}$") ///
		   drop(_I* ten* exp* oj $zlist _cons) ///
...
esttab iv_mm_young iv_mm_ten_young iv_cmm_mm_young ols_mm_young ols_mm_ten_young ols_cmm_mm_young ///
                   using ${result}/table_apx_${diminitls}_young.tex, b(4) se(4) ///
                   r2 nodepvars nogaps label longtable substitute(\hline\hline \hline \hline "\hline  " ///
		   "Standard errors in parentheses" "All regressions include occupation and industry dummies." ///
...
/* for referees */
/*------------------------------------------------------------------------------------*/		   
/* all dimensions robustness */
estimate clear
estimate use ${result}/iv_mm_dim.ster
estimate store iv_mm_dim
estimate use ${result}/iv_mm_ten_dim.ster
estimate store iv_mm_ten_dim
...
/*------------------------------------------------------------------------------------*/		   
/*------------------------------------------------------------------------------------*/
/* Wage growth early on and how much is due to mismatch's decline */
xtset id year

by id: egen wage0 = mean(lwage) if exp<= 2 & lwage<.
by id: egen wage_minage = min(wage0)
by id: gen cum_wage_growth_total = lwage - wage_minage if exp > 2 /*this will be very close actually to cum_excess_E*/
gen wage_growth = lwage - l.lwage
gen wage_growth_2 = f.lwage - l2.lwage
gen wage_growth_excess = wage_growth_2 - (f.wage_growth) - (l.wage_growth)
global xlist $xlist_0
global ivlist $ivlist_0
xi: ivregress 2sls wage_growth ($xlist = $ivlist) $zlist i.ind_1d i.occ_1d if switch_emp==0, vce(${vcetxt})
predict Ewage_growth
gen wage_growth_excess_E = wage_growth_2 - (f.Ewage_growth) - (l.Ewage_growth)


sum wage_growth_excess_E if switch_emp==1 & exp<=10

gen wage_growth_excess_E_swemp = wage_growth_excess_E if switch_emp==1
by id: gen cum_excess_E = sum(wage_growth_excess_E)
by id: gen cum_excess_E_swemp = sum(wage_growth_excess_E_swemp)

sum cum_excess_E_swemp if exp<=10, meanonly
local cum_swemp_mean = r(mean)
sum cum_excess_E if exp<=10, meanonly
local cum_mean = r(mean)

disp `cum_swemp_mean'/`cum_mean'

*now look at how occupation switchers contribute
gen mm_chng = mm - l.mm
gen wage_growth_excess_E_swocc = wage_growth_excess_E if switch_occ==1
gen wage_growth_excess_E_mm = wage_growth_excess_E if switch_occ==1 & mm_chng<0
by id: gen cum_excess_E_swocc = sum(wage_growth_excess_E_swocc)
by id: gen cum_excess_E_mm = sum(wage_growth_excess_E_mm)

sum wage_growth_excess_E_swocc if exp<=10 & mm_chng <0
local wg_E_mm = r(mean)
sum wage_growth_excess_E_swocc if exp<=10 & mm_chng >0
local wg_E_wrongmm = r(mean)

disp `wg_E_mm' - `wg_E_wrongmm'
disp `wg_E_mm'/`wg_E_wrongmm'

sum cum_excess_E_mm if exp<=10, meanonly
local cum_mm_mean = r(mean)
sum cum_excess_E if exp<=10, meanonly
local cum_mean = r(mean)
disp `cum_mm_mean'/`cum_mean'



drop wage0 wage_minage 
