...
/* drop if no valid health variables */
drop if hlth_composite == .
drop if height == .
drop if weight == .
drop if bdad_death == .
drop if bmom_death == .
...
/* physical ability */
reg hlth_composite I_enterage1-I_enterage8 if ind_indic == 1, robust
qui predict hlth_std, resid
sum hlth_std if ind_indic == 1
replace hlth_std = (hlth_std - r(min))/r(sd)  if ind_indic == 1
sort id year
replace hlth_std = hlth_std[_n-1] if id == id[_n-1]
gen ability_h = hlth_std
...
global hlth_list bmi_init_p25 bmi_init_m25 bmi_init2 height bdad_alive bdad_death bmom_alive bmom_death hispanic black
...
reg ability_h ${hlth_list} if ind_indic == 1, vce(robust)
...
esttab using ${result}/table_hlth_comp.tex, r2 nodepvars label wide se nonumbers gap mtitles("Health Index") ///
       ...
       order(bmi_init* height yob bdad_alive bdad_death bmom_alive bmom_death $zlist) ///
       star(* 0.10 ** 0.05 *** 0.01) replace 

predict hlth_fit, xb
sum hlth_fit if ind_indic == 1
replace ability_h = (hlth_fit - r(min))/r(sd) if ind_indic == 1
sort id year
replace ability_h = ability_h[_n-1] if id == id[_n-1]
*/
/*------------------------------------------------------------------------------------*/

/* export the correlation matrix between health and cog */
corr ability_v ability_m ability_s ability_h skill_v skill_m skill_s skill_h
matrix corr_vmp = r(C)
...
gen ability_dd = ability_h
gen skill_aa = skill_v
gen skill_bb = skill_m
gen skill_cc = skill_s
gen skill_dd = skill_h

gen ability_lev = (ability_aa + ability_bb + ability_cc + ability_dd)/4
...
gen skill_lev  = (skill_aa + skill_bb + skill_cc + skill_dd)/3
...
esttab iv_ind_mm_means iv_ind_mm_ten_means iv_ind_cmm_mm_means ols_ind_mm_means ols_ind_mm_ten_means ols_ind_cmm_mm_means ///
		   ...
		   drop(_I* ten* exp* oj $zlist _cons) ///
		   mtitles("IV" "IV" "IV" "OLS" "OLS" "OLS") ///
		   order(absmm_aa absmm_bb absmm_cc absmm_dd absmm_aa_t* absmm_bb_t* absmm_cc_t* absmm_dd_t* cmm_aa cmm_bb cmm_cc cmm_dd ability_?? ability_??_* skill_?? skill_??_*) ///
                   star(* 0.10 ** 0.05 *** 0.01) replace
...
esttab iv_ind_mm_means iv_ind_mm_ten_means iv_ind_cmm_mm_means ols_ind_mm_means ols_ind_mm_ten_means ols_ind_cmm_mm_means ///
		   ...
		   order(absmm_aa absmm_bb absmm_cc absmm_dd absmm_aa_t* absmm_bb_t* absmm_cc_t* absmm_dd_t* cmm_aa cmm_bb cmm_cc cmm_dd ability_?? ability_??_* skill_?? skill_??_* ten* exp* oj $zlist _cons) ///
                   star(* 0.10 ** 0.05 *** 0.01) replace
