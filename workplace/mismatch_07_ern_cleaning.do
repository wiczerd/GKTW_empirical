...
/* 
forvalues i=1/5 {
replace wage_emp`i'= wage_cps if check_cps_emp`i'==1 & (wage_cps > 0 & wage_cps !=. ) & (wage_emp`i' < 0 | wage_emp`i' == .)
                 }
*/
                                                                                                                             /* CHECK LATER */
...
save $data/ern_employment_02.dta, replace
...
sort id intvw_num
merge m:1 id intvw_num using $data/ern_employment_02
tab _m
drop if _m == 2
drop _m
...
gen wage  = wage_emp1  if lab_status == intvw_num*100 + 1

forvalues i=2/5 {
                 replace hrwkd = hrwkd_emp`i' if lab_status == intvw_num*100 +`i'
		 replace wage  = wage_emp`i'  if lab_status == intvw_num*100 +`i'
                 }

replace hrwkd = -10 if hrwkd ==.
replace hrwkd = -11 if lab_status ==0
replace wage  = -10 if wage  ==.
replace wage  = -11 if lab_status ==0

keep id time lab_status dual_emp* hrwkd wage
...
gen wage_d`i'  = wage_emp1  if dual_emp`i' == intvw_num*100 + 1

forvalues j = 2/5 {
replace hrwkd_d`i'= hrwkd_emp`j' if dual_emp`i' == intvw_num*100 + `j'
replace wage_d`i' = wage_emp`j'  if dual_emp`i' == intvw_num*100 + `j'
}

replace hrwkd_d`i' = -10 if hrwkd_d`i' ==.
replace hrwkd_d`i' = -11 if dual_emp`i' <= 0
replace wage_d`i'  = -10 if wage_d`i'  ==.
replace wage_d`i'  = -11 if dual_emp`i' <= 0

keep id time lab_status dual_emp* hrwkd hrwkd_d* wage wage_d*
}

sort id time

save $data/ern_weekly_02.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_weekly_02.dta, clear
...
/*creaes annual earnings */
gen earn = 0
replace earn = wage*hrwkd if wage > 0 & hrwkd > 0
forvalues i = 1/4 {
gen earn_d`i' = 0
replace earn_d`i' = wage_d`i'*hrwkd_d`i'
}

/* for each week chose the job that has been worked the highest number of hours */
gen earn_main  = max(earn,earn_d1,earn_d2,earn_d3,earn_d4)

gen job = lab_status
replace job = dual_emp1 if earn_main == earn_d1 & earn_main > earn /* & earn_main > 0 */                                    /* CHECK LATER */
replace job = dual_emp2 if earn_main == earn_d2 & earn_main > earn /* & earn_main > 0 */
replace job = dual_emp3 if earn_main == earn_d3 & earn_main > earn /* & earn_main > 0 */
replace job = dual_emp4 if earn_main == earn_d4 & earn_main > earn /* & earn_main > 0 */

gen hrwkd_main = hrwkd
replace hrwkd_main = hrwkd_d1 if earn_main == earn_d1 & earn_main > earn /* & earn_main > 0 */
replace hrwkd_main = hrwkd_d2 if earn_main == earn_d2 & earn_main > earn /* & earn_main > 0 */
replace hrwkd_main = hrwkd_d3 if earn_main == earn_d3 & earn_main > earn /* & earn_main > 0 */
replace hrwkd_main = hrwkd_d4 if earn_main == earn_d4 & earn_main > earn /* & earn_main > 0 */

drop lab_status dual_emp* hrwkd hrwkd_d* earn earn_d*

rename hrwkd_main hrs
rename earn_main earn_job
...
save $data/ern_weekly_03.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_weekly_03.dta, clear
...
keep id time year hrs fulltime intvw_num job job_number earn_job

save $data/ern_weekly_04.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_employment_02.dta, clear
...
save $data/ern_employment_03.dta, replace
...
save $data/ern_demographics_02.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
/* sample selection */

use $data/ern_weekly_04.dta, clear
...
merge m:m id year using $data/ern_demographics_02.dta
ta _m
drop _m
...
save $data/ern_weekly_05.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_weekly_05.dta, clear

sort id intvw_num
merge m:1 id intvw_num using $data/ern_employment_03.dta
ta _m
drop if _m == 2
drop _m
...
save $data/ern_weekly_06.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_weekly_06.dta, clear
...
save $data/ern_weekly_07.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_weekly_07.dta, clear
...
save $data/ern_yearly_01.dta, replace
...
save $data/ern_health_02.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/earnings_01.dta, clear
...
keep id year inc_salary inc_farm_biz

save $data/ern_earnings_02.dta, replace

/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/

use $data/ern_yearly_01.dta, clear

/*------------------------------------------------------------------------------------*/
/* merging health variable */

merge m:m id year using $data/ern_health_02.dta
...
merge m:1 id year using $data/ern_earnings_02.dta
drop _merge

/*------------------------------------------------------------------------------------*/

save $data/ern_yearly_02.dta, replace
