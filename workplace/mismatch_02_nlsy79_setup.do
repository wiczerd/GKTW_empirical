...
save $data/demographics_01.dta, replace
...
replace intvw_num = intvw_num - 1978 if intvw_num <= 1994
replace intvw_num = (intvw_num - 1994)/2 + 16 if intvw_num > 1994
/* saving employment data */
save $data/employment_01.dta, replace
...
/* saving demographics data */
save $data/weekly_01.dta, replace
...
save $data/health_01.dta, replace
...
save $data/social_01.dta, replace
...
clear
infile using $download/earnings/earnings.dct, clear

ssc install tolower

do $download/earnings/earnings-value-labels.do

/* create a variable list for later use*/
#delimit ;
local vlist_earnings
q13_5_
q13_9_
;
#delimit cr
...
/* Reshape original wide data from NLSY79 to long data */ 
reshape long `vlist_earnings', i(caseid_1979) j(year)

rename caseid_1979 id
rename q13_5_ inc_salary
rename q13_9_ inc_farm_biz
...
save $data/earnings_01.dta, replace
